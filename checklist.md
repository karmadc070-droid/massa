# massa 작업 체크리스트

## A. API 도메인 교체 (sslip.io → moahagwon.com 서브도메인)

목표: 앱이 임시 도메인 대신 정식 도메인을 바라보게 하고, IPv6 미지원 문제를 없앤다.

- [x] A1. Cloudflare에 `api.moahagwon.com` A+AAAA 추가 (DNS only) — 2026-08-29 완료
- [x] A2. Caddy `/root/Caddyfile`에 `api.moahagwon.com → localhost:8002` 추가, 무중단 reload — 완료
- [x] A3. IPv4/IPv6 모두 401 응답 확인 (인증서 자동 발급됨), 기존 sslip.io도 정상 — 완료
- [x] A4. index.html SUPABASE_URL 교체 → 로그인 255ms·목록 조회 정상 확인 — 완료
- [x] A5. DB URL 치환: providers.photo_url 25건 + providers.photo_urls(배열) 25건 = 50건 — 완료
- [x] A6. 심사 정보에는 서버 URL이 없어 갱신 불필요 — 확인함
- [x] A7. build 14로 1.0.1 심사 제출 완료 (2026-08-29) → 남은 verify: TestFlight 실기기에서 로그인·예약 동작

## A2. 비밀번호 재설정 메일 (Resend)

목표: 비밀번호를 잊은 사용자가 메일로 재설정할 수 있게 한다. GoTrue의 SMTP가 더미라 지금은 메일이 안 나간다.

- [x] A2-1. Resend 가입·도메인 `moahagwon.com` 추가, Auto configure로 Cloudflare에 DKIM·MX·SPF 등록 — 2026-08-29
- [x] A2-2. `massa.moahagwon.com` A 레코드(DNS only) + Caddy 정적 서빙 → `/srv/massa-web` (index.html, reset.html)
- [x] A2-3. `reset.html` 작성 (메일 링크로 들어와 새 비밀번호 설정) → verify: 200 응답 + 토큰 없이 열면 "만료" 안내 확인함
- [x] A2-4. index.html·admin.html 로그인창에 "비밀번호를 잊으셨나요?" 추가 → verify: 모듈 문법 검사 통과
- [x] A2-5. Resend API 키 발급 → VPS `/root/resend.key` (600)
- [x] A2-6. `scripts/vps-setup-smtp.sh` 실행 → verify: auth 컨테이너 env에 `GOTRUE_SMTP_HOST=smtp.resend.com`, `GOTRUE_SITE_URL=https://massa.moahagwon.com`, `GOTRUE_URI_ALLOW_LIST` 반영 확인함
- [x] A2-7. 실제 발송 확인 → Resend 로그 **Delivered**, 발신 `"massa" <massa@moahagwon.com>`
- [x] A2-8. 메일 링크 클릭 → `reset.html`이 recovery 세션을 받아 정상 폼 표시 (만료 안내 아님) — 전 과정 1회 성공

남은 개선

- [~] A2-9. 메일 템플릿 한국어화 — **사장님이 넘기기로 결정** (2026-09-05). 기본 영문 그대로 둔다
- [~] A2-10. Resend API 키 회전 — **사장님이 넘기기로 결정** (2026-09-05)
  - 남는 위험: 그 키로 `moahagwon.com` 발신 메일을 임의로 보낼 수 있다. 나중에 마음 바뀌면 Resend 콘솔에서 새 키 발급 후 `vps-setup-smtp.sh` 재실행

## B. 관리자·파트너 기능 웹 이전 (23개 화면)

목표: 앱은 고객 전용으로 남기고, 운영 기능은 `admin.moahagwon.com`에서 관리자 로그인 후 사용한다.

### B0. 준비 — 완료 (2026-08-29)
- [x] B0-1. `admin.html` 생성 (index.html 복사본 + `guardConsole` 권한 검사) → 미로그인 시 차단 화면 확인함
- [x] B0-2. 배포: VPS Caddy 정적 서빙. `admin.moahagwon.com` → `/srv/massa-admin`
  - DNS A 레코드 등록(DNS only), Caddy 블록 추가, noindex 헤더 포함
  - 배포 스크립트: `scripts/vps-deploy-admin.sh` (codeload로 최신 커밋 취득 + 모듈 스코프 검증)

### B1. 관리자 화면 13개 이전 — 완료 (2026-08-29)
adminApps(심사) · adminStats · adminReports · adminMembers · adminSettlement · adminAds · adminFraud · adminBookings · adminProviders · adminFee · adminCoupons · adminSales · adminCountry
- [x] B1-1. admin.html 에 13개 화면·로더 전부 존재함을 확인 (앱에서 지우기 전에 먼저 확인함)
- [x] B1-2. 앱에서 638줄 삭제 → verify: 문법 통과, 화면 34개 남음, div 균형 408/408, 남은 admin 참조 0

### B2. 파트너 화면 10개 이전 — 완료 (2026-08-29)
partnerBookings · partnerRevenue · partnerSchedule · partnerEvents · partnerStaff · partnerSettle · partnerChat · partnerGps · partnerAi · partnerAiBiz
- [x] B2-1. 앱에서 534줄 삭제 → verify: 문법 통과, 고객 화면 24개만 남음, div 356/356
- [x] B2-2. `isPartner`·`rowVis`·`canReview`·`FEE_RATE`/`DUE_DAYS` 등 고아 코드 정리
- [x] B2-3. 실제 배포 후 브라우저 확인 → 홈·목록 정상, 콘솔 오류 0
- index.html 334KB → 224KB

### B3. 마무리
- [ ] B3-1. demo 계정 파트너 권한 정리 여부 결정 (심사용으로 필요한지)
- [ ] B3-2. 앱 빌드·제출 → verify: 실기기에서 계정 화면에 운영 메뉴가 없음
- [ ] B3-3. 인계서(IOS_진행상황.md) 갱신

## D. 다국어 (한국어·베트남어·영어·중국어·일본어)

방침: 고객 앱은 5개 언어 전부(알림창 포함), 운영 콘솔은 한국어·베트남어.

### D1. 엔진 교체 — 완료 (2026-08-29)
- [x] D1-1. 정해진 CSS 클래스 순회 → **텍스트 노드 전체 순회**(`translateTree`)로 교체. 자식 태그가 있으면 건너뛰던 문제 해소
- [x] D1-2. `MutationObserver` 추가 → innerHTML 로 나중에 그려지는 목록·오버레이도 자동 번역
- [x] D1-3. `alert`·`confirm`·`prompt` 래핑 → 호출부 61곳을 고치지 않고 한 번에 번역 경로에 태움
- [x] D1-4. 선택한 언어를 localStorage 에 저장, 없으면 기기 언어를 따름 (이전에는 새로고침마다 한국어로 되돌아갔다)
- [x] D1-5. `trOne` 의 빈 문자열 폴백 버그 수정 (`e[idx] || ...` → undefined/null 만 폴백)

### D2. 사전 보강 — 완료
- [x] D2-1. DICT 202개 → **332개**, 중국어·일본어 누락 **56개 → 0개**
- [x] D2-2. TITLE_I18N 화면 제목 5개 추가 (계정 정보·제공자 등록 신청·지도 검색·지역 선택·채팅)
- [x] D2-3. 하노이 지명 7곳을 현지 표기로 (미딩 → Mỹ Đình 등)
- [x] D2-4. 재실행 가능한 스크립트로 관리 → `scripts/i18n-add.py`

### D3. 조합형 문구 — 완료
- [x] D3-1. 카드·목록의 조합 문구를 `<span>` 조각으로 분리 → 언어를 바꾸면 다시 그리지 않아도 그 자리에서 바뀜
- verify: 미번역 한국어 텍스트 노드 **190개 → 6~9개** (남은 것은 confirm/done 의 와이어프레임 더미로, 실제 예약 시 데이터로 대체됨)
- verify: 베트남어·일본어 실화면 확인 — 목록·계정 화면 전체 번역됨

### D4. 운영 콘솔 — 완료
- [x] D4-1. admin.html 에 같은 엔진·사전 이식, 언어 선택을 한국어·베트남어로 제한
- [x] D4-2. `guardConsole` 차단 화면도 번역 (load 보다 먼저 실행되므로 언어 복원을 IIFE 시작부에도 넣음)

### D5. 검증 — 완료 (2026-08-29)
- [x] D5-1. 예약 플로우 4개 화면(코스·시간·위치·확인)을 4개 언어로 밟음 → **남은 한국어는 확인 화면 요약 3줄뿐**이고 그건 i18n 문제가 아니라 아래 E1 버그
- [x] D5-2. alert/confirm 문구 23개를 사전에 등록. 뒤에 값이 붙는 문구(`저장 실패: ...`)는 `Tmsg()` 가 접두사만 바꾸도록 처리 → verify: 4개 언어 번역 결과 확인함
- [x] D5-3. 코스 카드의 `· 90분`, `(약 N원)`, 취소 안내의 시간 수치 등 조합 문구를 span 으로 분리

## E. 검증 중 발견한 결함 — 모두 수정 완료 (2026-08-29)

### E1. 예약 확인 화면이 선택 내용을 반영하지 않던 문제
`#confirm` 요약 3줄이 마크업에 하드코딩돼 있어, 다른 제공자·시간·호텔을 골라도 `Linh N. / 6월 7일(일) 20:00 / 롯데호텔 하노이 2104호` 가 그대로 보였다. (`submitBooking()` 은 선택값을 정확히 저장하므로 DB 는 정상이었고 화면만 어긋났다.)
- [x] E1-1. `fillConfirm()` 추가 → 서비스·제공자·날짜·장소·금액을 실제 선택값으로 채운다. 번역 조각은 span 으로 감쌈
- verify: 두 번째 제공자 + 다른 코스 + 수요일 14:30 + Grand Plaza Hanoi 1503 으로 4개 언어 확인 — 전부 정확히 반영됨

### E2. 위치 화면 호텔 입력란의 한국어 기본값
- [x] E2-1. `value="롯데호텔 하노이 (Ba Dinh)"` → `placeholder="예) 롯데호텔 하노이"`
- [x] E2-2. 기본값을 없앤 탓에 빈 채로 예약될 수 있어 `submitBooking()` 에 입력 검증 추가

### E3. 요일 번역이 날짜 숫자에 섞이던 버그 — 가장 위험했던 것
날짜 칩에서 `textContent.replace(/\D/g,'')` 로 날짜를 읽는데, 요일 라벨을 번역하자 **베트남어 수요일 `T4` 의 숫자 4 가 섞여** `06/10` 이 `06/410` 이 됐다.
- 확인 화면 표시뿐 아니라 **`submitBooking()` 의 저장 경로도 같은 코드**여서, 베트남어 사용자는 잘못된 날짜로 예약될 수 있었다.
- [x] E3-1. `selectedDayNum()` 을 만들어 `.dow` 를 뺀 텍스트에서만 숫자를 읽도록 수정, 두 곳 모두 교체
- verify: 4개 언어 모두 `06/10` 으로 정확히 나옴

## C. 기타 남은 일
- [ ] C1. build 12 실기기 로그인 확인 (2.1a 근본 원인 확정)
- [ ] C2. **안드로이드 TWA 가 낡은 Vercel 빌드를 본다 — C4 의 선행 조건** (2026-09-05 확인)
  - `android-package/massa.aab`(2026-08-14 업로드) 안의 시작 URL = `https://massa-seven.vercel.app/?src=pwa`
  - 그 Vercel 배포본(200 / 333KB)을 실제로 받아보니 **마이그레이션 이전 버전**이다
    | 표식 | Vercel(안드로이드가 보는 것) | massa.moahagwon.com(최신) |
    |---|---|---|
    | `nativeapp` (1.0.4 전체화면) | 0 | 4 |
    | `TOSS_ENABLED` | 0 | 2 |
    | `delete-account` | 0 | 있음 |
    | API 주소 | `massa.141-164-46-88.sslip.io` (구 주소) | `massa.moahagwon.com` |
    | `관리자` 문자열 | 45 | 27 |
  - `assetlinks.json` 은 **Vercel 도메인에만** 있다 (moahagwon·massaviet 는 404)
  - 즉 안드로이드 테스터 12명은 지금 **구버전 앱**을 쓰고 있다. 프로덕션 신청 전에 정리해야 한다
  - **①로 해결(2026-09-05).** Vercel 수동 배포 API 를 커밋 `a8b0cb8` 로 호출 → `READY`, `massa-seven.vercel.app` 별칭 연결
    - verify: 292,713 bytes 로 `massa.moahagwon.com` 과 **동일 크기**, `nativeapp` 4 / `TOSS_ENABLED` 2 / `CANCEL_RULES` 12
    - API 주소가 `massa.moahagwon.com` 으로 교체됨 (구 `sslip.io` 0건)
    - `delete-account`·`privacy`·`terms`·`pay-start`·`pay-return` 전부 200, `assetlinks.json` 200 (`app.massa.hanoi` 유지 → TWA 검증 안 깨짐)
  - Vercel 은 git 자동배포가 없다. 배포 트리거는 `PLAY_STORE_진행상황.md` §4 의 `/api/v13/deployments` 스니펫

### C2-2. TWA 를 자체 도메인으로 옮기는 건 (검토만, 2026-09-05)
사용자 요청 — "도메인이 있으니 Vercel 말고 여기로 다 옮기자". 확인 결과 **가능하지만 지금은 아니다.**
- **iOS 는 무관하다.** `capacitor.config.json` 이 `webDir: "www"` — 원격 URL 이 아니라 **번들 자산**을 쓴다
- 안드로이드만 해당. 시작 URL 은 aab 안에 박혀 있어 **bubblewrap 재빌드 + versionCode 증가 + Play 재업로드**가 필요하다
- 새 호스트에 `assetlinks.json`(지문 2개) 를 먼저 올려야 한다. 안 그러면 주소창이 뜨는 반쪽 TWA 가 된다
- 후보는 `app.massaviet.com`. 현재 앱 주소 `massa.moahagwon.com` 은 **학원 브랜드가 드러난다** — 도메인을 분리한 이유와 어긋나므로 옮길 값어치가 있다
- ⚠️ **API 호스트는 옮기면 안 된다.** 이미 출시된 iOS 1.0~1.0.3 에 `massa.moahagwon.com` 이 박혀 있어 끊으면 기존 사용자가 죽는다. 옮기는 건 **웹 호스트만**
- ⚠️ 프로덕션 신청(9/7)까지 2일이다. 재업로드가 14일 요건 계산에 영향을 주는지 확인되지 않았다.
  **콘텐츠 결함은 위에서 이미 해소됐으므로**, 신청을 먼저 하고 도메인 이전은 다음 릴리스로 미루는 편이 안전하다
- [x] C3. VPS DB 논리 백업 — **이미 돌고 있다** (2026-09-05 확인)
  - `crontab: 10 3 * * * /root/pg_backup.sh`, massa-db 포함 4개 DB 를 매일 03:10 덤프
  - `/root/backups` 에 10일치 보관, 디스크 26% 사용으로 여유 있음
  - ⚠️ 같은 서버에만 있다. 서버가 통째로 죽으면 같이 죽는다 — 외부 보관은 별도 과제
- [ ] C4. Google Play 프로덕션 신청 — 콘솔 확인(2026-09-05): **"현재 12일 동안 참여를 선택한 테스터 12명"**, 14일 요건까지 2일 남음
  - 신청 버튼은 비활성. 대략 **2026-09-07** 이후 활성화된다. 신청 시 비공개 테스트에 관한 서술형 질문에 답해야 한다
  - 승인 전까지 Play 스토어 URL 은 일반 방문자에게 404 다 (개발자 계정으로는 보인다 — 착각하기 쉽다)
- [ ] C5. Zalo 로그인 + Sign in with Apple (Zalo 계정 인증 후)

### D6. 동적 화면 추가 보강 (2026-08-29)
정적 마크업만 훑던 초기 점검이 **데이터를 불러와야 그려지는 화면**을 놓쳤다. 사용자가 프로필 상세 스크린샷으로 지적해 발견함.
- [x] D6-1. 제공자 프로필 상세 32개 문구 → 사전 등록 + 조합 문구(주소·리뷰 수·차단 토글) span 분리
- [x] D6-2. 알림 화면 6개 문구 → `notiAgo()` 의 '분 전/시간 전/일 전' 을 span 으로 분리
- [x] D6-3. 서비스 메뉴 이름 13종(오일 마사지 + 부항 요법 등) 사전 등록
- verify: 프로필 상세 4개 언어 **각 0개** — 미번역 없음
- **교훈**: 다국어 점검은 반드시 해당 화면을 열고 데이터를 로드한 상태에서 해야 한다. 정적 마크업 스캔만으로는 절반만 본다.

## F. 1.0.2 빌드·제출
- [x] F1. 1.0.1 심사 통과 → **출시 완료** (한국·베트남)
- [x] F2. `capacitor/package.json` 1.0.2 로 올림
- [x] F3. Codemagic **build 16** 업로드 성공 (build 15 는 프로필 다국어 수정 전이라 폐기)
- [x] F4. 1.0.2 버전 생성 → build 16 첨부 → 변경사항 작성 → **심사 제출 완료** (상태: 1.0.2 심사 대기 중)

## G. 소셜 로그인 (구글·카카오·애플)

방침: **키가 설정된 provider 만 버튼이 나타난다.** 앱이 `/auth/v1/settings` 를 읽어 판단한다.
예전에 "구글 버튼은 있는데 눌러도 안 됨"으로 Apple 4.0 리젝을 받았기 때문에, 키 없이 버튼이 노출되는 일이 없어야 한다.

### G1. 뼈대 — 완료 (2026-08-29)
- [x] G1-1. GoTrue 에 google·kakao·apple provider 매핑 추가 (docker-compose 12줄 + .env 자리 9개)
- [x] G1-2. 앱에 `loadSocialProviders()` / `renderSocialButtons()` / `socialLogin()` 추가
- [x] G1-3. 네이티브 복귀 처리 — `@capacitor/browser`(앱 내 브라우저) + `@capacitor/app`(appUrlOpen) + `onNativeAuthReturn()`
- [x] G1-4. iOS URL 스킴 `massa://` 등록을 codemagic.yaml 에 추가
- [x] G1-5. 키 발급 안내서 작성 → `소셜로그인_키발급_안내.md`
- verify: 키 없는 지금 → 버튼 **0개**. settings 응답을 켜진 것으로 바꾸면 → **3개**(구글·카카오·애플) 정상 표시

### G2. 키 발급

> 확장 프로그램(DeepSeek AI, 1688-aibuy) 때문에 **실제 마우스 클릭이 카카오 모달을 죽인다.**
> `javascript_tool` 로 pointer/mouse 이벤트를 직접 디스패치하면 통과한다. 구글 콘솔은 배율이 튀므로 ref 사용.

- [x] G2-1. 구글 — 프로젝트 `Massa`(massa-507208) 에 OAuth 구성 + 웹 클라이언트 `massa web` 생성 (2026-08-30)
  - 대상 **외부**, 게시 상태 **프로덕션** (테스트 모드면 등록한 계정만 로그인된다)
  - 리디렉션 `https://api.moahagwon.com/auth/v1/callback`
  - 브랜딩: 홈페이지·개인정보처리방침·약관 URL 등록, 승인된 도메인 `moahagwon.com`
  - ⚠️ 클라이언트 보안 비밀번호는 **생성 직후 1회만** 보인다. 사장님이 복사함
- [x] G2-2. 카카오 — 앱 `massa` (ID 1562938) 생성 (2026-08-30)
  - 카테고리 뷰티 / 회사명 massa / 대표 도메인 `https://massa.moahagwon.com`
  - 카카오 로그인 **ON**, Redirect URI 등록, 클라이언트 시크릿 **ON**(기본값)
  - 앱 아이콘 등록 (저장소 `icons/icon-192.png`) → **비즈 앱 전환의 선행 조건**이었다
  - 사업자 정보 등록 → **비즈 앱 전환 완료** (사장님이 직접 입력)
  - 동의항목 **닉네임 필수 동의**, **이메일 필수 동의 + 수집** (비즈 앱 전에는 `권한 없음`이라 못 켰다)
- [x] G2-3. 애플 — 전부 완료 (2026-08-30)
  - `app.massa.hanoi` 에 **Sign In with Apple 활성화** (프로파일 무효화 경고는 Codemagic 이 매 빌드 재생성하므로 무해)
  - Services ID **`app.massa.hanoi.web`** — Primary `massa hanoi`, Domain `api.moahagwon.com`, Return URL `https://api.moahagwon.com/auth/v1/callback`
  - Key **`massa Sign in with Apple`** (Key ID `884XVUF24R`) → .p8 을 VPS 로 scp → JWT 생성 → .p8·conf 삭제
  - **적용 완료: `/auth/v1/settings` 에서 `apple: true`**
  - Private Email Relay — `moahagwon.com` 등록, 상태 **SPF 통과** (Resend 가 세워둔 SPF 재사용, DNS 추가 작업 없었음)
  - ⚠️ **애플 시크릿 만료 2027-03-01.** 만료 전 `.p8` 로 다시 만들어야 로그인이 끊기지 않는다
  - ⚠️ developer.apple.com 은 App Store Connect 와 **세션이 따로**다. 다시 로그인해야 한다
- [x] G2-4. 애플 시크릿 JWT 생성 스크립트 `scripts/vps-apple-secret.sh` — .p8 을 VPS 안에서만 다루고 끝나면 삭제
  - verify: 테스트 EC 키로 서명 생성 → 공개키 검증 통과, 수명 182.6일(애플 상한 6개월 이내), sig 64바이트

### G3. 키 적용 후
- [x] G3-1. `.env` 채우고 auth 재기동 → **`google: true` / `kakao: true` / `apple: true`** (2026-09-04)
  - verify: 앱에서 버튼 3개 렌더 확인 — `Google로 계속하기` · `카카오로 계속하기` · `Apple로 계속하기`
  - 키 파일은 VPS·로컬 양쪽에서 삭제함
- [x] G3-2. 설정 검증 — 로그인 없이 확인 가능한 부분은 전부 통과 (2026-09-04)
  - 깨끗한 브라우저(세션 없음)로 `/auth/v1/authorize?provider=X` 를 직접 열어 각 provider 화면까지 도달 확인
  - 구글 → 로그인 화면 정상, 등록한 개인정보처리방침·약관 인식됨. `redirect_uri_mismatch`·`invalid_client` 없음
  - 카카오 → 로그인 화면 정상. `KOE006`(리디렉션 불일치)·`KOE101`(잘못된 앱 키) 없음
  - 애플 → "Apple 계정을 사용하여 **massa**에 로그인하십시오". Services ID·Return URL·Primary App ID 연결 확인
  - ⚠️ **남은 미검증 구간**: 실제 계정으로 동의한 뒤의 **토큰 교환**. 여기서만 검증되는 것 — 구글 시크릿 실값,
    카카오 시크릿 실값, **애플 JWT 와 .p8/Key ID 짝**. 실계정 로그인 1회로 확인해야 한다
- [ ] G3-3. iOS 빌드 올려 실기기에서 `massa://` 복귀 검증 — **build 17 진행 중 (1.0.3)**
- [ ] G3-4. 세 개 다 켠 상태로 심사 제출 (**애플 없이 구글·카카오만 켜서 iOS 제출하면 4.8 리젝**)

## I. 1.0.3 빌드·제출 (소셜 로그인)

- [x] I1. 1.0.2 출시 확인 — App Store 조회 API 로 `version 1.0.2`, `2026-08-30` 출시 확인
- [x] I2. `capacitor/package.json` 1.0.3 으로 올림
- [x] I3. 빌드 전 점검 — 소셜 코드 7곳 존재, `CFBundleURLSchemes` 등록 단계 존재,
  `TOSS_ENABLED=false`(토스 숨김 유지), native.js 호스트 라우팅 수정 반영
- [x] I4. Codemagic **build 17 성공** — 16단계 전부 통과. TestFlight `1.0.3 / build 17 / 제출 준비 완료`
- [x] I5. App Store Connect **1.0.3 버전 생성** → 변경사항 작성 → build 17 첨부 → 저장 완료
  - `심사에 추가` 버튼 활성 상태. **제출은 아직 누르지 않았다**
- [x] I6. 실기기(TestFlight)에서 버튼 3개 노출 확인 — 사장님 확인
- [x] I7. **토큰 교환까지 검증 완료** — 실계정 없이 확인했다 → `scripts/vps-verify-social-secrets.sh`
  - 원리: 토큰 엔드포인트에 일부러 틀린 code 를 보낸다.
    시크릿이 틀리면 `invalid_client`, 맞으면 `invalid_grant` 가 온다
  - 결과: 구글·카카오(KOE320)·애플 **셋 다 `invalid_grant`** = 클라이언트 인증 통과
  - 이걸로 **애플 JWT 와 .p8/Key ID 짝**까지 확인됐다. 서버에서 생성한 부분이라 가장 불확실했던 곳이다
  - 시크릿 값은 VPS 안에서만 읽어 화면에 출력하지 않는다
- [x] I8. **1.0.3 심사 통과·출시 완료** (2026-09-05)

## K. 1.0.4 — 화면이 꽉 차지 않던 문제 (2026-09-05)

**증상**: 앱을 켜면 화면에 꽉 차지 않고 좌우로 움직인다.

**원인**: `.phone` 은 데스크톱에서 볼 때 쓰는 **폰 목업 프레임**(테두리 10px, 둥근 모서리, 가짜 상태바)이다.
이걸 벗기는 규칙이 `@media (max-width: 767px)` 안에만 있어서 **세로 아이폰만** 적용됐다.

| 상황 | 수정 전 | 수정 후 |
|---|---|---|
| 아이폰 세로 375×812 | 375×812 정상 | 그대로 |
| **아이폰 가로 844×390** | 카드 315×560, 좌우 여백 257px, **높이가 화면보다 큼** | 844×390, 여백 0 |
| **아이패드 820×1180** | 카드 518×920, 좌우 여백 151px, 가짜 상태바 노출 | 820×1180, 여백 0 |

- [x] K1. `body.nativeapp` 클래스로 **화면 크기·방향과 무관하게** 목업을 벗기도록 수정
  - `IS_NATIVE_APP` 이 true 일 때만 클래스가 붙는다 → 웹은 그대로 (회귀 확인함)
  - 미디어쿼리는 손대지 않았다. 웹의 좁은 화면 동작은 기존과 동일
- [x] K2. 세 크기 전부 검증 — 여백 0, 가로 스크롤 없음, 뷰포트보다 넓은 요소 0개
- [x] K3. Codemagic **build 18 성공** → 1.0.4 생성·첨부 → **심사 제출 완료** (2026-09-05, 심사 대기 중)

> **왜 여태 안 보였나.** 세로 아이폰만 테스트했다. 가로로 돌리거나 아이패드로 열면 바로 드러나는 문제였다.
> 반응형은 **한 크기가 아니라 경계값**(767/768)을 넘겨보며 확인해야 한다.

## J. 비로그인 접근 점검 (2026-09-04)

"로그인 없이 다 클릭된다"는 지적을 받고 확인했다. **의도된 설계이고 문제 없다.**

- 둘러보기를 막지 않는 건 애플이 권하는 방향이다. 로그인 뒤에 콘텐츠를 숨기면 **5.1.1(v) 리젝** 사유가 된다
- 세션 없는 브라우저로 실제 호출 — 예약확정·결제·채팅·리뷰·신고·쿠폰·찜·예약내역 **전부 로그인창이 뜬다** (코드 내 가드 28곳)
- **DB 직접 검증** (앱을 거치지 않고 anon 키로 REST 호출)
  - 읽기: 예약·프로필·메시지·결제·서류·신고 전부 **빈 결과**. 남의 데이터가 새지 않는다
  - 쓰기: 6개 테이블 전부 **42501 (RLS 위반)** 로 차단
  - 전 테이블 RLS 켜짐, 정책이 `auth.uid()` 로 소유자 검사

> 남겨둔 것: `cancelBooking` 만 SESSION 검사가 없다. 예약 목록에서만 닿을 수 있고
> 비로그인은 목록이 비어 있어 도달 불가이며, 취소 시도 자체도 RLS 가 막는다. 실질 위험 없음.

> ⚠️ **테스트하다 헛짚은 것**: `toggleFav` 로 호출해 "가드 없음"으로 잘못 판단했다.
> 실제 함수명은 `toggleFavorite` 이고 가드가 있다. **함수명을 먼저 확인하고 호출할 것.**

> ⚠️ **구글 시크릿 함정.** 구글 콘솔은 생성 후 시크릿을 `****`로 가려서 보여준다.
> 그 표시값(8자)을 그대로 복사해 넣으면 버튼은 뜨는데 로그인이 실패한다 — 4.0 리젝을 부르는 상태다.
> 실제 값은 `GOCSPX-` 로 시작하는 35자다. 잃어버렸으면 `보안 비밀번호 추가` 로 새로 만들면 된다(기존 것은 유지된다).
> 적용 전에 **길이와 `*` 포함 여부를 반드시 확인할 것.**

## H. 토스 선불 결제

방침: 후불(현장 결제)은 그대로 두고 **선불 카드 결제를 선택지로 추가**한다. 상세는 `결제-연동-계획.md`.

- [x] H1(P1). DB — `payment_transactions` 16컬럼, `payment_method` enum 에 `toss`, RLS 2개
- [x] H2(P2). Edge Function `toss-payment` — create / confirm / cancel. 배포·8항목 검증 완료
- [x] H3(P3). 웹 중계 페이지 2개 (2026-08-30)
  - `pay-start.html` — 토스 SDK 로 결제창을 연다. 5개 언어 안내문
  - `pay-return.html` — 앱발 결제(`native=1`)는 `massa://pay` 로 되던지고, 웹은 그 자리에서 승인
  - `vps-deploy-web.sh` 에 두 파일 배포·내용검증·HTTP 확인 추가
- [x] H4(P4). 앱 복귀 처리
  - `startTossPayment()` / `onNativePayReturn()` 추가
  - **native.js 라우팅 버그 수정** — OAuth 실패와 결제 실패가 둘 다 `code=` 를 달고 와서
    파라미터로 갈라내던 기존 코드는 구글 PKCE 로그인을 결제 처리로 보냈다. 호스트(`auth`/`pay`)로 갈랐다
  - verify: 5개 URL 케이스(결제 성공·결제 실패·PKCE 코드·토큰·OAuth 거부) 전부 올바른 쪽으로 라우팅
- [x] H5(P5). 결제 시트에 `🔒 카드로 미리 결제` 추가. `TOSS_ENABLED=false` 로 **지금은 숨김**
  - 쿠폰은 결제 전에 `discount_vnd` 로 예약에 먼저 반영한다. 서버가 `amount_vnd - discount_vnd` 로 다시 계산하므로 화면 금액과 일치한다
- [ ] H6(P6). 토스 가입 → 테스트 키 입력 → `TOSS_ENABLED=true` → 전 구간 검증 → 라이브 키

> 결제 시트의 기존 `카드`·`QR`·`은행 이체` 는 **PG 연결 없이 `is_paid=true` 로 찍는 자리표시자**다.
> 이번 작업에서 건드리지 않았다. 토스가 살아나면 이것들을 어떻게 할지 따로 정해야 한다.

### 주의
- Zalo 는 이번 범위에서 제외 (계정 본인인증 후 별도 진행)
- `APP_SCHEME` (index.html) 과 codemagic.yaml 의 `CFBundleURLSchemes` 는 **항상 같은 값**이어야 한다

## L. massaviet.com 소개 사이트 + 검색 등록 (2026-09-05)

**왜 분리했나.** 모아학원은 학부모·아이 대상 학원 플랫폼이다. 마사지 서비스가 같은 도메인 아래 있으면
검색이나 주소로 연결이 드러났을 때 학부모 신뢰에 직접 영향을 준다. 애드센스 이전에 이 이유만으로 분리할 값어치가 있다.

- [x] L1. 도메인 `massaviet.com` 구매 (Cloudflare Registrar, 2027-09-05 만료, auto-renew ON)
  - 브랜드가 앞에 오는 `massaviet` 를 골랐다. 앱 이름이 massa 라 자동완성과 각인에 유리하다
- [x] L2. DNS 4개 레코드 — apex·www 각각 A(141.164.46.88) / AAAA. **DNS only**(Caddy 가 인증서를 직접 발급)
  - ⚠️ Cloudflare DNS UI 가 좌표·React 양쪽으로 계속 어긋났다. **대시보드 세션으로 `/api/v4` 를 직접 호출**해 해결
- [x] L3. 소개 사이트 10개 페이지 — 홈·서비스·이용안내·안전검증·FAQ·파트너·회사소개·문의·약관·개인정보
  - 공용 `style.css`, 페이지마다 title·description·canonical·OG, 홈에 LocalBusiness / FAQ 에 FAQPage 구조화 데이터
  - verify: 10개 전부 메타 6항목 통과, div 균형, 내부 링크 깨짐 0
- [x] L4. VPS 배포 — `scripts/vps-deploy-massaviet.sh`, Caddy + Let's Encrypt
  - verify: 12개 경로 전부 200, `www` → apex **301**
- [x] L5. `robots.txt`(사이트맵 선언) · `sitemap.xml`(10 URL)
- [x] L6. **구글 서치콘솔** — 도메인 속성으로 등록, TXT 를 Cloudflare API 로 넣어 **"소유권이 자동으로 확인됨"**
- [x] L7. **빙 웹마스터** — `massaviet.com` 추가, `msvalidate.01` 메타 태그를 홈에 심어 확인 완료
- [x] L8. **사이트맵 제출** — 구글 "사이트맵이 제출됨", 빙 `Success / 11 URL 발견`
  - 도메인 속성은 `sitemap.xml` 만 넣으면 "사이트맵 주소가 잘못됨"이 뜬다. **전체 URL**을 넣어야 한다
  - 입력이 안 먹던 이유는 필드가 아니라 **위에 떠 있던 홍보 툴팁이 클릭을 가로챈** 것이었다. 툴팁을 닫으니 바로 됐다
- [ ] L9. 네이버 서치어드바이저 등록 — **자동화 불가**. 브라우저 양쪽 모두 `searchadvisor.naver.com` 접속이 차단된다
  - 사용자가 직접 사이트 추가 후 **메타 태그 content 값만** 넘겨주면 배포·확인은 내가 한다 (빙과 같은 방식)
- [ ] L10. 다음(카카오) 검색 등록 — **자동화 실패**. `_jsSubmit` 이 어떤 URL 형식도 "URL을 정확히 기입하여 주십시요."로 반려
  - `https://`, `http://`, 스킴 없음, `www` 포함 6가지 전부 실패. 검증 함수 소스는 확장 필터에 막혀 읽지 못했다
  - 어차피 2단계가 **개인정보수집·소유권 동의**라 사용자 승인이 필요하고, 마사지 업종은 **제출 서류**를 요구할 수 있다
- [ ] L11. 애드센스 신청 — **콘텐츠가 더 쌓인 뒤에**

### L12. 사이트 ↔ 앱 연결 (2026-09-05)
- [x] `site/download.html` 신설 — App Store 버튼(실링크) + Google Play(`.store.off`, 링크 없음)
- [x] 8개 페이지 헤더 nav · 푸터에 "앱 다운로드" 추가, 홈 히어로 1순위 CTA 를 다운로드로 교체
- [x] 홈에 스토어 버튼 섹션, `LocalBusiness` 구조화 데이터에 `sameAs` 로 App Store URL
- [x] `sitemap.xml` 11 URL 로, 배포 스크립트 검증 목록에 `download.html` 추가
- verify: 13개 경로 전부 200, 홈 applelink=2 / playlink=0, 8개 페이지 nav 전파 확인
- **Play 링크를 일부러 걸지 않았다.** 프로덕션 트랙이 아직 `12일 / 14일 필요`라 공개 URL 이 없다.
  개발자 계정으로는 스토어 페이지가 보이지만 일반 방문자에겐 404 다. 죽은 다운로드 버튼은 예전에 겪은 4.0 리젝과 같은 결함이다
- 프로덕션 승인 뒤 할 일: `site/download.html` · `site/index.html` 의 `.store.off` 두 곳을 실링크로 교체

> **취소 규정 불일치를 잡았다.** 소개 사이트 초안에 "2회 경고·3회 제한"으로 썼는데,
> 앱의 실제 값(`CANCEL_RULES`)과 약관은 **3회 경고·5회 누적 7일 제한**이었다.
> 추측으로 쓴 숫자였고, 실제 상수를 읽어 guide·faq 양쪽을 고쳤다.

> **애드센스 관련 솔직한 판단.** 지금 10페이지는 "회사 소개형"이라 애드센스가 저품질로 볼 여지가 있다.
> 하노이 마사지·홈뷰티 관련 실질 정보 글이 몇 편 더 쌓인 뒤 신청하는 편이 승인 확률이 높다.
> 또한 massa 는 수수료 10% 로 버는 사업이라 광고 수익 자체는 크지 않다 — 기대치를 낮게 잡는 편이 낫다.

## M. massa bot 정비 (2026-09-05)

**전제 정정.** "massa 에 챗봇을 만들자"로 시작했으나 **봇은 이미 있다**. `index.html` 3038~3168행,
FAB `openBot()` + 규칙 FAQ 15개 + 자연어 검색(`parseSearch` 7종 조건 → `CACHE.providers` 필터).
내가 `botReply` 로만 검색해 0건을 보고 "없다"고 했던 것이 오판이었다. 실제 함수명은 `botAnswer`·`openBot` 이다.
따라서 이 작업은 신규 구축이 아니라 **기존 봇의 결함 정비**다. LLM 은 붙이지 않기로 했다(사장님 선택).

### 발견한 결함
- **M-a 봇이 한국어로만 말한다.** 답변 14개 + 퀵리플라이 6개 = **20개 전부 `DICT` 미등록**.
  앱의 나머지 문구는 5개 언어가 다 있는데 봇만 빠졌다. 하노이 서비스라 베트남어 사용자가 가장 큰데 치명적이다.
- **M-b 키워드 매칭도 한국어 전용.** `has('예약','어떻게'...)` 라서 베트남어·영어로 물으면 **어떤 규칙에도 안 걸리고**
  기본 문구로 떨어진다. 답변만 번역해서는 해결되지 않는다.
- **M-c 퀵리플라이를 번역하면 검색이 깨진다.** 칩 텍스트가 그대로 `respond()` 로 들어가는데
  `parseSearch` 는 한국어 정규식('만','커플','여성전용')이다. 번역된 칩을 누르면 파싱이 실패한다.
  → **표시는 번역문, 전달은 한국어 원문**으로 분리해야 한다.
- **M-d 문의 경로가 끊겨 있다.** 봇은 "계정 → 회사 소개의 연락처"를 안내하는데
  회사 소개는 `alert()` 한 줄(3295행)이고 연락처가 없다. 사이트가 안내하는 `support@massaviet.com` 은
  **MX 레코드가 없어 반송**된다. 사용자가 문의할 방법이 실제로 없다.
- **M-e 가격이 하드코딩.** "아로마 90분 850,000₫" 을 봇이 단언한다(3044행). DB 값이 아니라 상수다.
- **M-f 취소 규정이 뭉뚱그려져 있다.** "다음 예약이 제한될 수 있습니다" — 실제는 `CANCEL_RULES`
  3회 경고 / 5회 누적 7일 제한 / 30일 창. 사이트·약관과 표현을 맞춰야 한다.

### 작업
- [x] M1. FAQ 를 `BOT_FAQ` 배열로 재구성 — 규칙 13개 / 키워드 171개, 5개 언어 전부 검사
  - **순서가 우선순위다.** 좁은 의도(취소·쿠폰·가격·내 예약)를 넓은 의도(예약 방법)보다 앞에 뒀다
  - verify: 25건 시뮬레이션 전부 통과, 여러 규칙에 중복된 키워드 0
- [x] M2. 퀵리플라이 — 표시 `T(ko)` / 전달 `ko` 분리
  - verify: 칩 6개 전부 한국어 원문으로 `parseSearch` 통과 (커플·한국인 인기·발마사지·10만원 이하)
- [x] M3. 봇 문구 35개를 `scripts/i18n-add.py` 로 DICT 등록 → verify: 미등록 0건
- [x] M4. 가격을 `CACHE.services` 최저가로 계산 (하드코딩 850,000₫ 제거)
- [x] M5. 취소 답변을 `CANCEL_RULES` 에서 생성 → 3회/5회/7일/30일
- [x] M6. 문의 경로 복구 — 회사 소개 alert 에 `SUPPORT_EMAIL`·massaviet.com 추가, 봇도 실제 주소 안내
- [x] M7. Cloudflare Email Routing 개통 — `support@massaviet.com` → 개인 메일 전달
  - verify: 외부 DNS(1.1.1.1)에서 MX 3개(`route1~3.mx.cloudflare.net`) + `v=spf1 include:_spf.mx.cloudflare.net` 확인
  - 웹 A 레코드(141.164.46.88)는 그대로 — 사이트에 영향 없음
- [x] M8. 배포 후 라이브에서 5개 언어 확인 — UI·인사·칩·placeholder 전부 해당 언어로 렌더

> **조각 이어붙이기로 만든 문장이 깨졌다.** 숫자가 들어가는 답변을 `T('예약') + h + T('시간 전까지…')`
> 식으로 조립했더니 베트남어에서 **"5lần thì 7bị hạn chế đặt lịch trong ngày"** 처럼 어순이 무너졌다.
> 언어마다 숫자 위치가 다른데 한국어 어순을 고정해 버린 탓이다.
> **문장 전체를 사전에 넣고 `{h}`·`{w}` 자리표시자만 치환**하도록 바꿔 해결했다(`botFill`).
> 조각 키 10개는 다른 문구를 오역할 소지가 있어 DICT 에서 삭제했다.
> 코드를 눈으로 봤을 때는 멀쩡해 보였고, **실제로 돌려보고서야 드러났다.**

> **번역 품질 주의.** 5개 언어 번역은 내가 작성한 것이다. 특히 베트남어는 주 사용자층 언어이므로
> 원어민이 한 번 훑어보는 편이 좋다. 문장 구조는 검증됐지만 어감까지 보장하지는 못한다.

## N. 전체 점검 (2026-09-05)

사장님 요청으로 인프라·앱·DB·메일을 실제로 돌려 확인했다.

### 정상 확인
- 웹 엔드포인트 **16개 전부 200** (앱·admin·massaviet·Vercel·법적 페이지·결제 릴레이·assetlinks)
- 컨테이너 **12개 전부 healthy**, 디스크 26%, 인증서 전부 11~12월 만료(여유)
- 백업 **4개 DB 매일 03:10**, massa-db 포함 33개 보관
- 소셜 로그인 **apple·google·kakao 전부 `true`**
- 익명 RLS — 공개 카탈로그만 읽히고(providers·services·stores·reviews),
  개인 데이터(bookings·profiles·messages)는 `[]`, 쓰기는 4개 테이블 전부 **42501**
- 취소 규정 **4개 면 일치** (앱 상수·앱 봇·사이트 guide/faq·약관 = 3회/5회/7일/30일)
- 가격 하드코딩 **0건**
- Cloudflare Email Routing **enabled / status=ready / DNS동기화됨**, support 규칙 활성

### 찾아서 고친 것

- [x] **N1. 로그인 오버레이가 통째로 미번역이었다** — 가장 큰 결함
  - `openAuth()` 의 15개 문구(제목·안내·입력 placeholder·버튼·소셜 3종·오류 문구)가 전부 `DICT` 에 없었다
  - **예약하려는 비한국어 사용자가 처음 만나는 화면**인데 한국어로 떴다. 하노이 = 베트남어가 주 사용자층이다
  - 봇만 고쳤을 때 "다국어 됐다"고 넘어갈 뻔했다. 화면을 실제로 훑어보고서야 드러났다
  - verify: 라이브에서 베트남어로 전환 후 확인 — 제목·부제·placeholder·버튼·소셜 전부 베트남어

- [x] **N2. 안드로이드 TWA 에 1.0.4 전체화면 수정이 적용되지 않고 있었다**
  - `body.nativeapp` 은 `IS_NATIVE_APP`(Capacitor)일 때만 붙는다. **안드로이드는 TWA 라 false** 다
  - 남은 건 `@media (max-width: 767px)` 뿐이라 **가로 모드(폭 767 초과)·태블릿에서 폰 목업 테두리와
    가짜 상태바가 그대로 노출**된다. iOS 1.0.4 에서 고친 것과 똑같은 증상이 안드로이드에만 남아 있었다
  - 수정: `@media (display-mode: standalone/fullscreen/minimal-ui)` 추가. TWA·PWA 는 standalone 으로 뜨므로
    플래그 없이 잡힌다. 일반 브라우저 탭은 `browser` 라 영향 없음
  - verify: CSSOM 에 규칙 3개 파싱됨, 브라우저 탭에서는 `현재매치=false` 로 목업 유지(회귀 없음)
  - VPS·Vercel 양쪽 재배포 완료 → 안드로이드에 반영됨

### 확인 못 한 것 / 남은 것
- [ ] N3. `support@massaviet.com` **실제 수신 테스트** — VPS 25번 포트가 막혀 서버에서 발송 불가.
  사장님이 아무 계정에서 한 통 보내보시면 확정된다
- [ ] N4. iOS 1.0.4 심사 상태 — App Store Connect 세션이 만료돼 확인 못 했다
- `zalo-login` 이 500 을 낸다 — 버그가 아니라 `ZALO_APP_ID 미설정`(C5 미완료). 앱 UI 에서 도달 불가
- 앱 전 화면 다국어 스윕은 완주하지 못했다. 사장님 크롬의 1688 번역 확장이 DOM 을 계속 주입해
  MutationObserver(60ms 마다 `translateTree(body)`)와 겹치면서 렌더러가 멈췄다. 확장 없는 환경에서 다시 볼 것

## O. massaviet.com 전면 재설계 (2026-09-06)

사장님 요청 — awwwards 를 참고해 깔끔하고 엘레강스한 사이트로 다시 만들고 앱을 연동한다.
정한 방향: **따뜻한 에디토리얼 / 한국어·영어·베트남어 / 10페이지 전체 재설계**.

### 구조를 바꿨다
10페이지 × 3언어 = 33개를 손으로 관리할 수 없어 **생성기 방식**으로 갔다.
- `site-src/content_{ko,en,vi}.py` — 페이지를 **블록 데이터**로 기술 (hero·section·grid·pcards·steps·table·faq·pull·note·stores)
- `site-src/build.py` — 블록을 HTML 로 찍고 hreflang·sitemap·robots·FAQ 구조화 데이터를 자동 생성
- `site-src/style.css` — 디자인 시스템 한 곳
- `site/` 는 **자동 생성물**이다. 직접 고치지 말고 content 를 고친 뒤 `python3 site-src/build.py` 를 돌린다
- ko 는 루트, 영어는 `/en/`, 베트남어는 `/vi/`. 33개 URL 이 서로를 hreflang 으로 가리킨다

### 이미지
사이트가 이미지를 **하나도 안 쓰고 있었다.** 그게 싸구려로 보이던 가장 큰 이유다.
- `masaage1.png` 를 히어로로 썼다 — 유일하게 문구가 안 박힌 깨끗한 사진이다
- `banner2·3` 은 왼쪽에 한국어가 박혀 있어 **오른쪽만 잘라** 홈뷰티·스파 카드로 썼다
- 전부 WebP 로 변환, 5장 합쳐 **184KB**
- ⚠️ `11~99.jpg`(아오자이 인물사진 9장)는 **쓰지 않았다.** 350×450 이라 히어로로 못 쓰고,
  출처 불명 실존 인물이라 초상권 문제가 있으며, 무엇보다 마사지 서비스 홈에 젊은 여성 얼굴을
  격자로 깔면 massa 가 쌓아온 포지셔닝과 정면으로 어긋난다

### 만들면서 잡은 것
- [x] **한글 제목이 엉뚱하게 끊겼다** — `max-width: 12ch` 가 원인. `ch` 는 라틴 폰트의 '0' 폭 기준이라
  82px 한글에 469px 밖에 안 준다. max-width 를 없애고 줄바꿈은 콘텐츠의 `<br>` 로 정하도록 바꿨다.
  `word-break: keep-all` 도 넣었다 — 없으면 한글이 단어 중간에서 끊긴다
- [x] **한글에 세리프가 안 먹었다** — Cormorant 에는 한글 글리프가 없어 시스템 고딕으로 떨어지고
  라틴/한글 인상이 따로 놀았다. `Noto Serif KR` 을 추가했다
- [x] **"3단계 검증"의 3 이 아래첨자처럼 보였다** — Cormorant 가 올드스타일 숫자를 기본으로 쓴다.
  제목에만 `font-variant-numeric: lining-nums` 를 걸었다
- [x] **CSS 수정이 반영되지 않았다** — Caddy 가 10분 캐시한다. 내용 해시를 `style.css?v=…` 로 붙여 해결
- [x] **배포 스크립트가 `img/` 를 못 옮겼다** — `cp` 가 디렉터리를 건너뛰었다. `cp -R` 로 바꾸고
  배포 전 대상 폴더를 비우도록 했다(배포본 = 저장소 상태)
- [x] **모바일에서 언어 전환이 사라졌다** — `nav.main a:not(.navcta)` 가 언어 링크까지 숨겼다.
  베트남 사용자 대부분이 모바일인데 언어를 못 바꾸는 상태였다. 직계 자식만 숨기도록 고쳤다

### 앱 연동
- 헤더 CTA·홈 스토어 섹션·전용 다운로드 페이지, 3개 언어 전부
- 모바일에서만 뜨는 **설치 유도 띠**(한 번 닫으면 localStorage 로 기억)
- Play 는 여전히 링크를 걸지 않는다(`PLAY_LIVE=False`). 프로덕션 승인되면 `build.py` 의 그 값만 True 로 바꾸고 다시 빌드

### 곁다리로 잡은 사실 오류
개인정보처리방침이 처리 위탁을 **"Supabase Inc."** 로 적고 있었다. 자체 호스팅으로 옮긴 뒤라 사실과 다르다.
서버 위치를 확인하니 Vultr(The Constant Company) **서울 리전**이라 지역 표기는 맞았다.
수탁자를 실제 사업자로 정정하고 Resend·Cloudflare 도 추가했다(3개 언어 모두, 버전 1.1).

### 검증
- 33개 페이지 전부 200, 깨진 링크 0, 필수 메타 6항목 전부 통과
- hreflang 상호 참조 3언어 × 4개(ko·en·vi·x-default) 확인
- FAQ 구조화 데이터 언어당 18문항 자동 생성
- sitemap 33 URL / 99 alternate
- 390px 에서 가로 스크롤 없음, 넘치는 요소 0, 언어 전환·CTA·설치 띠 정상

### 남은 것
- [ ] O1. **베트남어 원어민 검수** — 번역은 내가 썼다. 하노이 현지 고객과 파트너 모집에 쓰이는 언어이므로
  실제 홍보 전에 한 번 훑어보게 하는 편이 좋다. 문장 구조는 맞지만 어감까지 보장하지 못한다
- [ ] O2. 구글 서치콘솔·빙에 사이트맵 재크롤 확인 (URL 은 그대로라 자동으로 잡히지만 며칠 걸린다)
- [ ] O3. 사진이 3장뿐이라 페이지마다 같은 이미지가 반복된다. 실제 서비스 사진이 생기면 교체할 것

## P. 수수료 입금 관리 (2026-09-06)

**해결한 문제.** 현금 거래가 대부분이라 마사지사가 회사 계좌로 수수료를 이체하는데,
**동명이인이 있어 통장만 봐서는 누가 냈는지 알 수 없다.**

### 방식
제공자마다 `MS####` 고유 코드를 발급하고, 이체 메모(또는 입금자명)에 적게 한다.
관리자 화면에 코드가 함께 뜨므로 통장과 1:1 대조된다.

### 스키마 — `scripts/settlement_schema.sql` (저장소에 DDL 을 남겼다)
기존 `settlement`·`settlement_cycle` 은 **DDL 이 저장소에 없고 VPS 에만 있었다.** 새로 만드는 것은 파일로 남긴다.
- `providers.deposit_code` — `MS` + 4자리, unique. 27명 전원 자동 발급
- `provider_deposit` — 입금 신고·확인. `kind`(commission|prepay), `status`(reported|confirmed|rejected), `memo`, `proof_url`
- `provider_credit` / `provider_credit_tx` — 선입금 잔고와 증감 내역 (페이백용, 지금은 안 씀)
- `owns_provider(uuid)` — 본인 소유 판정 (프리랜서 `profile_id`, 샵 `owner_id` 양쪽)
- `confirm_deposit(uuid)` RPC — 관리자만. 수수료면 사이클을 `paid` 로, 선입금이면 보너스까지 잔고에 적립
- RLS — 제공자는 자기 것만 조회·신고, 확인은 관리자만

### 화면
- **관리자(정산 관리)** — 이름 옆에 코드 배지, 마사지사가 올린 신고를 노란 박스로 띄우고
  `통장 확인함·승인` / `확인 안 됨`(사유 입력 → 알림 발송) 버튼
- **마사지사(내 입금 현황)** — 자기 코드를 크게 보여주고 `입금했습니다` 로 금액·메모를 신고, 처리 상태 표시
- 하드코딩 `10%` 라벨을 `FEE_RATE` 로, `3일` 을 `DUE_DAYS` 로 바꿨다 (설정과 화면이 어긋나던 것)
- `admin.html` 에 이스케이프 헬퍼가 없어 `esc()` 를 만들었다 — 이체 메모는 사용자 입력이다

### 선입금(페이백) — 스키마만, 기능은 끔
`app_settings.prepay` 에 `enabled:false` 와 3구간을 넣어 뒀다. **금액은 임시값이다.**
- 200만₫ 이상 → 보너스 10% / 500만₫ 이상 → 15% / 1,000만₫ 이상 → 20%
- 90분 아로마 850,000₫ 기준 건당 수수료가 85,000₫ 이라 월 20건이면 1.7M₫ 정도. 그 기준으로 잡은 값이다
- 실제 운영 전에 사장님이 금액을 정해야 한다

### 검증 — DB 에서 전 과정을 돌렸다
| 항목 | 결과 |
|---|---|
| 수수료 신고 → 승인 | `confirmed`, `settlement.status = paid` 로 전이 |
| 선입금 500만₫ + 보너스 75만₫ | 잔고 **5,750,000₫**, 내역 `prepay`/`bonus` 2건 |
| 같은 건 두 번 승인 | 잔고 그대로 (멱등) |
| 비관리자 승인 시도 | `관리자만 확인할 수 있습니다` 로 차단 |
| 테스트 데이터 | 전부 삭제, 남은 행 0 |

### 관리자 메일
- [x] `karmadc070@gmail.com` — admin 권한 있음 (2026-08-29 가입)
- [ ] **P1. `moahagwon@gmail.com` 은 아직 massa 계정이 없다.** 앱이나 admin 콘솔에서 그 메일로
  가입하시면 admin 권한을 부여하겠다
- [ ] **P2. Cloudflare 수신 주소 확인 필요.** `moahagwon@gmail.com` 을 수신 주소로 등록했고
  확인 메일이 갔다. 그 메일함에서 링크를 눌러야 `확인 대기` → `확인됨` 이 된다
  - ⚠️ Cloudflare Email Routing 은 **규칙당 액션 1개, 액션당 주소 1개**만 허용한다.
    한 주소를 두 곳으로 보내려면 같은 matcher 로 규칙을 하나 더 만들어야 하는데,
    수신 주소가 확인되기 전에는 생성 자체가 거부된다. 확인 후 다시 시도할 것
- [x] **P3. 관리자 알림 메일 — 만들었다** (2026-09-06)

### P3. 알림 메일 구조
`functions/notify-admin/index.ts` — Resend 로 두 관리자 주소에 동시 발송. action 3가지.
- `deposit_reported` — 입금 신고가 들어오면 즉시. 마사지사·코드·금액·메모를 표로
- `daily_digest` — 매일 00:00 UTC(하노이 07:00). 확인 대기 + 기한 초과 목록. **처리할 게 없으면 안 보낸다**
- `test` — 설정 점검용

**클라이언트가 아니라 DB 가 호출한다.** `scripts/notify_trigger.sql` 에서 `provider_deposit` insert 트리거가
`pg_net` 으로 함수를 부른다. 앱이 죽거나 우회해도 알림이 빠지지 않는다.
알림 실패가 입금 신고 저장을 막지 않도록 `exception when others` 로 감쌌다.

**보안** — 함수는 공개 엔드포인트라 `x-notify-secret` 공유 시크릿으로 막았다.
없으면 누구나 메일을 쏠 수 있는 중계기가 된다. verify: 시크릿 없이 호출 → **401**

**함정 하나.** `.env` 에 키를 넣었는데 컨테이너가 못 읽었다.
`docker-compose.yml` 의 `functions.environment:` 에 **명시된 변수만** 들어간다.
SMTP 때 겪은 것과 똑같은 함정이라 `scripts/vps-fix-notify-env.sh` 로 4개 매핑을 추가했다.

**검증**
| 항목 | 결과 |
|---|---|
| `test` 발송 | Resend 200, `sent_to` 두 주소 |
| 시크릿 없이 호출 | **401** |
| 입금 신고 insert → 트리거 | `pg_net` 응답 **200**, 메일 발송 확인 |
| 크론 등록 | `0 0 * * * /root/massa_digest.sh` |
| 빈 상태 다이제스트 | `처리할 건이 없어 보내지 않음` |
| 테스트 데이터 | 전부 삭제, 남은 행 0 |

### P4. support@ 를 두 주소로 보내는 건 — Cloudflare 로는 안 된다
두 주소 모두 **확인 완료**됐지만, 같은 matcher 로 규칙을 하나 더 만들면 `Duplicated Zone rule` 로 거부된다.
Cloudflare Email Routing 은 **규칙당 액션 1개 · 액션당 주소 1개 · 주소당 규칙 1개**다.
- 현재: `support@massaviet.com` → `karmadc070@gmail.com` 만
- 우회 ①(권장) `karmadc070` Gmail 설정에서 `moahagwon` 으로 자동 전달 — 사장님이 Gmail 에서 몇 번 클릭
- **우회 ② Email Worker — 시도했으나 안 됐다. 되돌렸다** (2026-09-06)
  - Worker(`massa-mail-fanout`)를 올려 `support@` 규칙을 `worker` 액션으로 바꿨다. API 는 전부 200
  - 그런데 메일이 도착하지 않았다. **고객 문의가 유실될 수 있어 즉시 직접 전달로 복구**하고,
    `wtest@massaviet.com` 이라는 별도 주소로 옮겨 시험했다 (support@ 는 건드리지 않은 채로)
  - Workers Analytics: 호출 **1회 · status success · errors 0**. 그런데 4분이 지나도 미도착
  - 원인 미확정. 내 Worker 코드가 `try/catch` 로 `forward` 실패를 삼켜서 **"success" 가 배달을 증명하지 못한다**.
    다시 한다면 예외를 삼키지 말고 그대로 던져 analytics 에 errors 로 잡히게 해야 한다
  - 시험 규칙과 Worker 를 **모두 삭제**했다. 남은 규칙은 `support@ → karmadc070` 하나뿐이고 워커는 0개
  - ⚠️ **support@ 수신 자체는 아직 한 번도 실증되지 않았다.** 내가 연결된 Gmail 은 `moahagwon@gmail.com`
    (보낸편지함이 비어 있고, moahagwon 앞으로 온 메일만 보인다)이라 karmadc070 도착 여부를 볼 수 없다.
    사장님이 아무 계정에서 `support@massaviet.com` 으로 한 통 보내 karmadc070 에 오는지 확인해 주셔야 한다
- 우회 ② catch-all 을 `moahagwon` 으로 돌리면 support@ 외 주소만 그쪽으로 간다 (support@ 는 여전히 한 곳)
- **시스템 알림 메일은 이 제약과 무관하다** — Resend 로 보내므로 이미 두 주소에 모두 간다


### P5. support@massaviet.com — **정상이었다.** 내 시험 방법이 틀렸다 (2026-09-06)

> **결론 먼저.** 사장님이 실제 메일 클라이언트로 보낸 메일은 **도착했다.**
> 내가 "수신이 안 된다"고 판단한 것은 오진이었고, 원인은 **시험을 전부 Resend 로 보낸 것**이다.
>
> | 시험 | 발신 | 결과 |
> |---|---|---|
> | B·C·F | **Resend** | 전부 미도착 |
> | 사장님 실제 발송 | 일반 메일 클라이언트 | **도착** |
>
> Resend → massaviet.com(Cloudflare MX) 경로만 배달되지 않는다. 발송 전용 키라 Resend 쪽
> 배달 상태를 조회할 수 없어 더는 좁히지 못했다. **고객 문의 수신에는 영향이 없다** —
> 고객은 일반 메일 클라이언트로 보낸다.
>
> **교훈**: 시험 경로가 실제 경로와 같은지 먼저 따졌어야 했다. Resend 는 우리 시스템이
> *보낼* 때 쓰는 경로지, 고객이 *보내는* 경로가 아니다. 다른 것을 시험해 놓고 결론을 냈다.
> 앞서 `toggleFav`(없는 함수)를 부르고 "가드 없음"이라 한 것, 로그인된 브라우저에서
> Play 스토어가 열리는 걸 보고 "공개됐다"고 한 것과 같은 종류의 실수다.

#### 그때 확인해 둔 설정 (전부 정상)

**증상.** 발송은 성공하는데 도착하지 않고, **바운스도 오지 않는다.** 50분 이상 기다려도 마찬가지.

**확인한 것 — 설정은 모든 축에서 정상이다**
| 항목 | 상태 |
|---|---|
| zone | `active`, 네임서버 Cloudflare |
| MX | route1·2·3.mx.cloudflare.net 3개 (외부 DNS 1.1.1.1 에서도 조회됨) |
| SPF | `v=spf1 include:_spf.mx.cloudflare.net ~all` |
| Email Routing | `enabled=true` · `status=ready` · `synced=true` |
| 규칙 | `support@massaviet.com` → forward, **priority 0**, enabled |
| catch-all | **비활성** (drop 이지만 꺼져 있어 이 규칙에 걸리지 않는다) |
| 수신 주소 | karmadc070·moahagwon **둘 다 verified** |

**시험한 것**
- A. Resend → support@ → karmadc070 : 확인 불가 (내가 볼 수 없는 메일함)
- B. Resend → wtest@ → Worker → 두 주소 : **미도착**. Worker 는 호출 1회·success·errors 0
- C. Resend → support@ → **moahagwon** (내가 볼 수 있는 곳으로 돌려서) : **미도착** ← 가장 결정적
- D. Gmail(moahagwon) → support@ → moahagwon : 미도착이지만 **이 시험은 무효다.**
  Gmail 은 같은 Message-ID 를 중복 제거하므로 자기 자신에게 되돌아오는 메일은 안 보일 수 있다. 설계 실수였다

C 가 실패한 것이 핵심이다. **Worker 문제가 아니라 massaviet.com 인바운드 자체가 배달되지 않는다.**

**남은 가설** — Email Routing 을 켠 지 1시간이 안 됐다. 프로비저닝이 덜 끝났을 가능성.
Resend 키가 발송 전용이라 배달 상태를 조회할 수 없어 더 좁히지 못했다.

**현재 상태** — `support@ → karmadc070@gmail.com` 원상 복구. 시험용 Worker·규칙은 전부 삭제.

- [x] P5-1. 수신 확인 완료 — 사장님 실제 발송분 도착
- [x] **P5-2. 문의함을 내가 볼 수 있게 바꿨다.** `support@massaviet.com` → **`moahagwon@gmail.com`**
  - 이 세션에 연결된 Gmail 이 moahagwon 이라, 이제 고객 문의를 내가 직접 읽고 처리할 수 있다
  - `forward` 액션은 실증된 방식이다 (worker 액션은 불안정해서 뺐다)
- [~] P5-3. 두 주소 동시 수신은 접었다. Cloudflare 는 주소당 규칙 1개·액션 1개·대상 1개다.
  Worker 팬아웃을 두 번 시도했는데 첫 번째는 호출됐으나 미배달, 두 번째는 **호출조차 되지 않았다**.
  `worker` 액션 경로가 불안정하다고 보고 시험 규칙·스크립트를 전부 삭제했다.
  karmadc070 에서도 받고 싶으면 **moahagwon Gmail 에서 자동 전달**을 걸면 된다 (사장님 소유 계정)
  - ⚠️ **시스템 알림 메일은 이 제약과 무관하다.** Resend 로 직접 보내므로 이미 두 주소에 모두 간다

---

## Q. 관리자 지표 대시보드 (2026-09-06)

사장님 요청 — "massa 앱에서 사람들 유입량과 마사지 매칭율, 어느 마사지사가 인기가 좋은지
수익은 어떻게 되는지, Daily·주간·월간·년간 등 보고 싶다. 단, 관리자만 볼 수 있어야 한다."

### 착수 전 실측
| 항목 | 실제 |
|---|---|
| bookings | 7건, **전부 `confirmed`**, 전부 2026-08-25 하루, `completed` 0건 |
| 수익 지표 | 계산식은 맞지만 현재 값은 0 (완료 건이 없어서) |
| `events` 테이블 | 프로모션용이지 접속 기록이 아니다 |
| 유입량 원천 | **없었다.** 새로 계측해야 했다 |
| 관리자 계정 | `karmadc070@gmail.com` 하나뿐 |

- [x] Q1. **유입 기록 테이블** `app_visit` — 날짜 + 기기 랜덤 ID 로 하루 한 줄.
      IP·이름·이메일은 저장하지 않는다. RLS 를 켜고 **정책을 하나도 만들지 않아** PostgREST 로는 아무도 못 읽는다.
- [x] Q2. **집계 함수** `admin_dashboard(bucket, periods)` — `security definer`.
      호출자가 `profiles.role='admin'` 이 아니면 `42501` 로 거절한다. 한 번 호출로 한 화면을 다 채운다.
- [x] Q3. **앱에 계측 심기** — `index.html` 의 `load()` 가 끝난 뒤 `trackVisit()` 1회.
      로그인 여부까지 담아야 해서 `refreshSession()` 이후로 잡았다.
- [x] Q4. **화면** `massaviet.com/admin/` — 일간(14)·주간(12)·월간(12)·연간(5) 탭,
      KPI 6개(유입·가입·예약·매칭율·거래액·수수료), 막대 차트 3개, 인기 마사지사 표.
      차트는 라이브러리 없이 SVG 로 직접 그린다.
- [x] Q5. **색인 차단** — `robots.txt` 에 `Disallow: /admin/`, 페이지에 `noindex`, sitemap 제외.
      다만 **이건 보안 장치가 아니다.** 권한은 DB 가 본다.
- [x] Q6. **개인정보처리방침 갱신** — 앱·사이트(ko/en/vi) "이용 통계" 항목 추가.
- [x] Q7. **Vercel 재배포** — 안드로이드 TWA 가 보는 `massa-seven.vercel.app` 갱신.

### 검증 결과 (전부 통과)
| 시험 | 결과 |
|---|---|
| psql · 관리자로 호출 | 200, jsonb 반환 |
| psql · 일반 고객으로 호출 | `ERROR 관리자만 조회할 수 있습니다` |
| psql · anon 으로 `app_visit` 직접 읽기 | 0행 |
| PostgREST · 관리자 JWT | HTTP 200 |
| PostgREST · 고객 JWT | HTTP 403 (42501) |
| PostgREST · 비로그인 | HTTP 401 |
| PostgREST · `track_visit` 비로그인 | HTTP 204 (열려 있어야 정상) |
| 브라우저 · 실제 배포본 렌더링 | KPI·차트 3개·표 정상, 빈 데이터일 때도 안 깨짐 |
| 브라우저 → DB 왕복 | localStorage 기기 ID `7e940e07…` 이 `app_visit` 에 그대로 기록됨 |
| 배포 md5 | 로컬 `site/admin/index.html` 과 서버 파일 동일 |

시험용 방문 기록 3건은 지웠다 (실제 유입 숫자를 오염시키면 안 된다).

- [x] Q8. **notify 시크릿 회전** — 진단 중 `app_settings.notify.secret` 값이 작업 로그에 찍혀
      새 값으로 교체했다. 새 값 200 / 옛 값 401 확인.
      ⚠️ 이때 `/root/massa_digest.sh` 에 시크릿이 **박혀 있다**는 걸 놓쳐서 크론이 401 이 될 뻔했다.
      회전 스크립트가 다이제스트 스크립트까지 다시 쓰도록 고쳤다.

### 남은 것
- iOS 는 `www/` 를 번들에 넣으므로 **다음 빌드 때** 유입 계측이 들어간다. 지금은 웹·안드로이드만 집계된다.
- 완료(`completed`) 예약이 아직 0건이라 수익·인기 마사지사는 값이 비어 있다. 지표가 아니라 데이터 문제다.

### Q9. 관리자 로그인 수단 (2026-09-06)

사장님이 "비밀번호를 어떻게 입력하냐, 관리자 비번을 만들어 달라"고 하셨다.
**비밀번호를 만들어서 알려 드리지 않았다.** 그 값이 작업 기록에 남기 때문이다.
대신 계정 상태를 먼저 확인했다.

| 확인 | 결과 |
|---|---|
| `karmadc070@gmail.com` 비밀번호 | **이미 있다** (마지막 로그인 2026-09-04) |
| 연결된 수단 | `email` + **`google`** |
| 구글 로그인 서버 설정 | `enabled=true`, client_id 있음 |
| ⚠️ `GOTRUE_URI_ALLOW_LIST` | **massaviet.com 이 빠져 있었다** → 로그인 후 되돌아오지 못했을 것 |

- [x] Q9-1. 허용 목록에 `https://massaviet.com/*` 추가 후 auth 재기동. authorize 302 확인.
- [x] Q9-2. 로그인 화면에 **Google 버튼** 추가 (`redirectTo: https://massaviet.com/admin/`).
- [x] Q9-3. **비밀번호 재설정** 링크 추가 — 이메일 입력 후 누르면 재설정 메일 발송,
      링크는 이미 동작하는 `massa.moahagwon.com/reset.html` 로 돌아온다.
- [x] Q9-4. 구글에서 돌아올 때 주소에 남는 `?code=` 를 지운다.

**참고** — 허용 목록에 없는 주소로 authorize 를 불러도 구글까지는 302 가 간다.
GoTrue 는 **콜백 시점에** 검사해서 허용 목록 밖이면 `SITE_URL` 로 되돌린다. 정상 동작이다.

### Q10. 구글 로그인이 루트로 튕기며 토큰이 주소창에 노출됐다 (2026-09-06)

사장님이 로그인 후 `massaviet.com/#access_token=eyJ...` 주소를 그대로 보내 주셨다.
**두 가지 문제가 겹쳤다.**

**(1) 복귀 주소가 `/admin/` 이 아니라 루트였다.**
GoTrue 허용 목록의 글롭에서 `*` 는 **경로 구분자 `/` 를 넘지 못한다.**
`https://massaviet.com/*` 는 `https://massaviet.com/admin/` 에 걸리지 않는다.
걸러지자 GoTrue 가 referer(사이트 루트)로 되돌려 보냈다.
`auth` 로그의 `referer: https://massaviet.com/` + `path:/callback` 이 증거다.
→ 허용 목록에 **`https://massaviet.com/**`** 추가. `**` 는 여러 단계 경로를 덮는다.

**(2) 암시적(implicit) 흐름이라 토큰이 주소창 해시에 그대로 붙었다.**
그래서 주소를 복사하는 것만으로 관리자 세션이 통째로 새어 나갔다.
→ `createClient` 에 `flowType: 'pkce'` 를 명시. 이제 주소창에는 1회용 `?code=` 만 남는다.

**대응**
- [x] `karmadc070@gmail.com` 의 세션 4개, 리프레시 토큰 55개 전부 삭제.
- [x] 허용 목록 `**` 추가 후 auth 재기동.
- [x] `flowType: 'pkce'` 적용·배포. 로그인 화면 정상, 콘솔 오류 없음.
- ⚠️ **노출된 액세스 토큰 자체는 취소할 수 없다.** JWT 는 상태를 보지 않아
  발급 후 1시간(02:50 UTC)까지 유효하다. 그 안에 누군가 쓰면 관리자 권한으로 조회가 된다.
  완전히 무효화하려면 `JWT_SECRET` 을 갈아야 하는데, anon·service 키까지 전부 재발급해
  앱·사이트에 다시 배포해야 한다. 만료가 임박해 그 비용이 더 크다고 보고 하지 않았다.

**교훈** — 로그인 흐름은 기본값을 믿지 말고 `pkce` 를 명시할 것.
그리고 허용 목록에 하위 경로가 있으면 `*` 가 아니라 `**` 를 쓸 것.

## R. 대시보드 재설계 · 가격표 (2026-09-06)

### R1. 가격 정리용 엑셀 — `massa_가격표.xlsx`
`services` 60건을 3개 탭으로. 수식 152개, 오류 0.

| 탭 | 내용 |
|---|---|
| 현재 가격표 | 60행 · 분당 단가 자동 계산 · **F열 노란 칸에 새 가격을 넣으면 변동률이 나온다** |
| 시간대별 비교 | 기본 서비스 × 30·45·60·90·120분 매트릭스 + 60→120 배수. 빈칸이 곧 없는 상품 |
| 정리 필요 | 결정해야 할 11건 |

**찾아낸 것** — 이름만 다르고 값이 완전히 같은 짝이 넷(다리=풋, 머리=헤드,
목·어깨=어깨·목=등, 타이=태국식). 아로마 마사지와 아로마테라피는 전 시간대 10만동 차이.
120분이 대부분 850,000 으로 눌려 60분의 등급 차이가 사라진다.
그래서 다리 마사지는 분당 단가가 60분 5,833 → 120분 7,083 으로 **길수록 비싸진다.**

⚠️ 마운트된 폴더 위에서 `recalc.py` 를 돌리면 LibreOffice 가 멈춘다(OOM 로 컨테이너째 죽었다).
`/tmp` 로 옮겨 돌린 뒤 결과만 복사할 것.

### R2. 대시보드 재설계 — Finebank 참고, massa 색 유지
사장님 선택: **밝은 massa 톤 유지 + 대시보드 문법**. 우선순위는 오늘 → 매칭 → 수익 → 인기 순.

- [x] R2-1. 함수 확장 — 직전 같은 길이 기간(`prev`), 오늘 스냅샷(`today`),
      취소·노쇼·대기 분리, 마사지사 사진·지역까지 반환.
- [x] R2-2. 좌측 어두운 사이드바(잉크) + 스크롤 위치 따라가기.
- [x] R2-3. 상단 **오늘 스트립** — 유입·가입·새 예약·완료·확인 대기. 확인 대기가 1건 이상이면 주황.
- [x] R2-4. KPI 4장 — 증감 배지 + 스파크라인.
- [x] R2-5. 수익 영역 차트 · 매칭 도넛(성사·대기·취소·노쇼) · 유입 막대 · 마사지사 사진 카드.

**설계에서 잡은 두 가지**
1. **비율의 증감은 %가 아니라 %p 다.** 76.5% → 76.8% 은 "0.3%p 상승"이지 "0.3% 상승"이 아니다.
   매칭율 카드만 `deltaPoint` 를 쓴다.
2. **수수료 선을 뺐다.** 수수료는 거래액의 정확히 10% 라 선을 하나 더 그으면 모양이 겹칠 뿐 정보가 없다.
   그래프는 거래액 하나만, 수수료는 설명줄에 숫자로 적는다.

**검증**
| 시험 | 결과 |
|---|---|
| 실제 배포본에 가짜 데이터 주입 | KPI·도넛(합계=예약 수 일치)·차트·카드 정상 |
| 거의 빈 데이터 | 매칭율 100%·"비교 없음"·"예약이 없습니다" 로 안전하게 처리 |
| 좁은 화면 경로 | viewBox 460×250 로 전환, 축 라벨 6개로 솎임 |
| SQL 키 ↔ JS 키 대조 | 어긋난 키 없음 |

## S. 수수료 3단계 등급 (2026-09-06)

사장님 결정 — 가게 소속 15% · 개인 프리랜서 20% · 인기 좋은 사람 10%.

**착수 전에 확인한 것**
- `providers` 에 가게/프리랜서 구분 컬럼이 **없었다.** `facility` 는 "개인 룸·샤워 시설" 같은 시설 설명 텍스트다.
- `stores` 는 제휴 매장 안내용(골프·호텔·네일샵)이고 마사지사와 잇는 키가 없다.
- 수수료율이 예약에도 정산에도 기록되지 않았다. 계산할 때마다 전역 10% 를 가져다 썼다.

**사장님이 정한 세 가지**
1. 10% 는 **자동 추천만, 적용은 수동.** 요율이 저절로 오르내리면 분쟁이 생긴다.
2. 기존 27명은 **전원 20%(프리랜서)** 로 시작. 확인되는 대로 내려 준다.
3. **예약 시점 요율로 고정.** 등급을 바꿔도 지난 정산 금액은 움직이지 않는다.

- [x] S1. `fee_tier` enum(vip·shop·freelancer) + `providers.fee_tier` (기본 freelancer, 변경 이력 남김).
- [x] S2. `bookings.fee_rate` 추가 + **예약 생성 트리거**로 그 시점 요율을 박는다.
      기존 7건은 실제로 10% 로 계산돼 왔으므로 0.10 으로 채웠다.
- [x] S3. `close_settlement_cycle` 을 **예약별 반올림 후 합산**으로 고쳤다.
      요율이 섞이면 합계에 요율 하나를 곱하는 방식은 틀린다.
- [x] S4. `admin_provider_fees()` — 등급 목록과 10% 후보. 후보 기준은 `app_settings.fee.promote`
      (최근 30일 완료 20건 이상 + 평점 4.7 이상). 기준값은 설정에서 바꾼다.
- [x] S5. `set_fee_tier()` — 관리자만. 누가 언제 바꿨는지 기록.
- [x] S6. 대시보드가 **DB 가 예약별로 더한 수수료**를 그대로 쓰도록 수정. 인기 마사지사에 등급 배지.
- [x] S7. 운영 콘솔 정산 화면에 **등급 버튼 3개 + 10% 후보 안내**. 파트너·매출 화면도 예약별 요율로.

**검증 (전부 통과)**
| 시험 | 결과 |
|---|---|
| 초기값 | 27명 전원 freelancer 20% |
| 기존 예약 | 7건 모두 0.10 으로 굳음 |
| 새 예약 | 0.20 이 박힘 |
| 등급을 vip 로 변경 | 요율 0.10 |
| **과거 예약** | **0.20 그대로 — 움직이지 않음** |
| 변경 후 새 예약 | 0.10 |
| **정산 합계** | **300,000 = 20%1건 + 10%1건. 정확** |
| 권한 | 고객은 목록·변경 모두 42501 거부 |
| 화면 계산식 | 섞인 요율 합계 700,000 정확. 옛 방식이면 **166,667₫ 틀렸을 것** |

⚠️ 앞서 토큰 노출 때 세션을 전부 끊었으므로 **운영 콘솔에 다시 로그인**해야 한다.

## T. 오픈 기념 일괄 10% · 마사지사 명단 · 페이백 (2026-09-06)

사장님 지시 — 3단계 기준(10·15·20%)은 그대로 두고, **오픈 기념으로 지금은 전원 10%.**
그리고 대시보드에 가입 명단을 넣어 사람별 받을 금액과 페이백을 다루게.

- [x] T1. 27명 전원 `vip`(10%)로 전환. **새로 승인되는 사람도 10% 로 들어오게 기본값을 바꿨다.**
      오픈 행사가 끝나면 `alter table providers alter column fee_tier set default 'freelancer'` 로 되돌린다.
      **등급 정의와 기준은 그대로 살아 있다.** 개별로 15·20% 로 바꾸면 즉시 그렇게 적용된다.
- [x] T2. `admin_provider_list()` — 승인·심사대기 전원. 이름·사진·등급·가입일·지역·입금코드,
      예약 수, 발생 수수료, 확인된 입금, 선입금 잔액, **미납**, 확인 대기 건수, 10% 후보 여부.
- [x] T3. `admin_add_prepay()` — 관리자가 통장에서 확인한 선입금을 직접 넣는다.
      보너스를 비우면 `app_settings.prepay` 구간(200만 10% · 500만 15% · 1000만 20%)에서 자동 계산.
      신고 단계를 건너뛰고 바로 확정해 잔고에 적립한다.
- [x] T4. `admin_use_credit()` — 잔고에서 수수료를 뺀다. **자동으로 깎지 않는다.** 관리자가 누를 때만.
      잔고보다 큰 금액은 막는다.
- [x] T5. 대시보드에 **마사지사 명단** 표 추가 (사이드바 항목도 분리 — 인기 순위 / 마사지사 명단).
      이름·지역·코드로 검색. 각 행에 [페이백 입력], 잔고가 있으면 [잔고 차감].

**미납 정의** — `발생한 수수료 − 확인된 수수료 입금 − 선입금 잔액`. 음수면 0.

**검증 (전부 통과)**
| 시험 | 결과 |
|---|---|
| 일괄 전환 | 27명 전원 vip · 기본값 `'vip'::fee_tier` |
| 명단 | 27명 반환 |
| 미납 계산 | 100만 예약(10%) → 발생 100,000 · 미납 100,000 |
| 페이백 200만 입력 | 보너스 20만 자동 · 잔고 220만 |
| 미납 덮임 | 잔고가 더 크면 미납 0 |
| 잔고 차감 10만 | 210만 |
| 잔고 초과 차감 | 거부 |
| 권한 | 고객은 명단·페이백 모두 42501 거부 |
| 화면 | 3행 렌더 · 검산 일치(1,705,000−900,000=805,000) · 잔고 있는 행만 [잔고 차감] · 검색 동작 |

**남겨 둔 것**
- `admin_provider_fees()` 는 운영 콘솔이 아직 쓰고 있어 그대로 뒀다. 대시보드는 `admin_provider_list()` 를 쓴다.
  나중에 콘솔도 옮기면 앞의 것을 지울 수 있다.
- 정산 마감 시 선입금 잔고에서 **자동 차감하는 흐름은 넣지 않았다.** 지금은 관리자가 눌러야 빠진다.

## U. GLOW 가격 벤치마킹 (2026-09-06)

경쟁사 = **glowvietnam.com** (BK Vietnam Technology JSC, 하노이). 같은 업종의 출장 마사지 앱이다.

**수집 방법** — GLOW 는 기사가 각자 값을 정한다. 그래서 한 명만 보면 안 된다.
하노이 공개 프로필 **40명**을 긁어 서비스×시간별 최저·중앙·최고를 냈다.
massa 60개 항목에 대응표를 붙여 비교했다.

### 핵심
- **GLOW 하노이의 사실상 정가는 60분 500,000 · 90분 600,000 · 120분 700,000.**
  60분 기준 500,000 이 22명, 600,000 이 16명, 700,000 이 2명.
  **시간이 늘 때마다 정확히 10만동씩** 올린다.
- **massa 는 60분이 싸고 120분이 비싸다.** 평균으로는 GLOW 와 +0.1% 로 같아 보이지만
  그건 두 방향 차이가 상쇄된 결과다. 평균만 보면 놓친다.
  - 60분: 다리·머리·풋 350,000 = **-30%**
  - 120분: 대부분 850,000, 핫스톤·아로마테라피 950,000 = **+21% ~ +36%**
- **분당 단가가 거꾸로 간다.** massa 다리 마사지 60분 5,833₫/분 → 120분 7,083₫/분.
  GLOW 는 8,333 → 5,833 으로 내려간다. 보통 장시간은 할인한다.
- **귀청소가 크게 싸다.** massa 30분 200,000 대 GLOW 40분 400,000~500,000. 분당 6,667 대 10,000.
- **한국식 전신 스크럽도 싸다.** massa 60분 500,000 대 GLOW 600,000~700,000 (-17~29%).
- **대응 없음 10건** — 네일·젤·각질·부위별 왁싱 일부는 GLOW 공개 프로필에 없다.

### 산출물 — `massa_GLOW_가격비교.xlsx` (수식 162개, 오류 0)
| 탭 | 내용 |
|---|---|
| GLOW 비교 | 60행 · massa 가격 옆에 GLOW 최저·중앙·최고 · 중앙 대비 % · **K열에 새 가격 입력** |
| 시간대별 나란히 | 60·90·120분을 massa/GLOW 로 붙여 놓고 60분·120분 차이를 계산 |
| 무엇이 보이나 | 위 핵심 9건을 결정 사항 형태로 |

⚠️ 공개 프로필 기준이다. 앱 안 실제가·할인은 다를 수 있으니 큰 결정 전에는 GLOW 앱에서 직접 볼 것.

## V. 가격 개편 앱 반영 (2026-09-06)

사장님이 새 가격표 60건을 주셨다. **가격만 바꿔서는 반영이 안 되는 문제 두 개**를 함께 잡았다.

### V1. 가격 적용
- 마사지 전 항목 **60분 500,000 · 90분 600,000 · 120분 700,000** 으로 통일 (GLOW 하노이 시세).
  44건 변경. 60분은 최대 +43%(다리·머리·풋 350,000→500,000), 120분은 -18~-26%.
- 홈뷰티·케어 13건은 값 그대로.
- **풀 왁싱 90분 1,200,000 신규**. 비키니는 45분 유지(50은 오타로 확인).
- '왁싱 60분'은 풀 왁싱이 대신하므로 **내림(is_active=false)**. 예약이 `service_id` 를 참조해 지우지 않았다.

### V2. ★ 앱에서 39개를 고를 수 없었다
`massage_type` 이 아로마·스웨디시·타이 7건에만 있었다. 나머지 39건(다리·등·머리·핫스톤·스포츠…)은
**앱 화면에 나오는 경로 자체가 없었다.** 2026-07-10 에 일괄로 넣고 앱과 연결하지 않은 것이다.

- [x] enum 에 `hot_stone·foot·head·back·neck_shoulder·sports·cupping·no_oil` 추가.
- [x] 이름만 다른 중복 5쌍(풋=다리, 헤드=머리, 어깨·목=목·어깨, 태국식=타이, 아로마 마사지=아로마테라피)
      → 하나만 남기고 15건 내림.
- [x] **코스 화면을 데이터 기반으로 다시 짰다.** 하드코딩 카드 3개 → `renderMenu()` 가 DB 를 읽어 그린다.
      앞으로 종류를 늘리거나 내려도 HTML 은 안 고쳐도 된다.
- [x] 새 코스 이름·설명 26개를 4개 언어 사전에 추가.
- **결과: 11종 × 60·90·120분 = 33개, 전부 앱에서 선택 가능.**

### V3. ★ 청구 금액 버그 두 개
1. **스웨디시·타이에 120분이 없는데** 앱은 120분을 고르면 조용히 60분짜리를 잡았다.
   → 손님이 120분을 골랐는데 **500,000₫ 으로 예약**됐다. 120분을 700,000 으로 추가하고,
   시간이 정확히 맞는 것만 쓰도록 `currentService()` 의 폴백을 없앴다.
2. **`amount_vnd: svc?.price_vnd || 850000`** — 코스를 못 찾으면 **850,000₫ 을 임의로 물렸다.**
   손님이 고르지 않은 금액이다. 폴백을 없애고 코스가 없으면 예약을 막는다.
- [x] `is_active` 필터 추가 (index.html 1곳 · admin.html 2곳). 내린 상품이 고객에게 보이면 안 된다.

### 검증
| 시험 | 결과 |
|---|---|
| DB | 활성 47(마사지 33 + 홈뷰티 14) · 내림 16 · 종류 없는 마사지 0 |
| 실제 앱 90분 | 11개 코스 전부 600,000₫ |
| 60분 / 120분 | 500,000 / 700,000 정확 |
| 다국어 | 베트남어·한국어 모두 새 코스 이름 번역됨 |
| 코스 선택 | 클릭하면 `SEL.serviceType` 이 바뀜 |
| 배포 | VPS + Vercel(안드로이드) 모두 옛 폴백 0건 확인 |

⚠️ **iOS 는 다음 빌드부터** 반영된다. `www/` 를 앱에 넣고 나가는 구조다.

### V4. 가격 개편 뒤 훑다가 잡은 것 (2026-09-06)
- [x] 완료 화면의 예시 금액이 `850,000₫` 로 박혀 있었다 → 600,000 으로. (index.html · admin.html 둘 다)
- [x] **관리자 예약 화면의 상태 필터가 실제 값과 달랐다.**
      칩이 `pending`·`accepted` 였는데 `booking_status` enum 에는 그런 값이 없다.
      → 그 필터를 누르면 **늘 0건**이었고, 지금 있는 7건(confirmed)은 **라벨 없이 영문 그대로** 나왔다.
      칩을 요청됨·확정·진행 중·완료·취소·노쇼로, 라벨도 7개 전부 맞췄다.
- [x] 존재하지 않는 `status='accepted'` 분기 2곳 제거.
- 사이트(massaviet.com)에는 금액이 박혀 있지 않아 손댈 것이 없었다.
- 미사용 사전 항목 `'예상 금액 850,000₫'` 이 남아 있다. 쓰이지 않아 그대로 뒀다.

## W. iOS 1.0.5 빌드 (2026-09-06)

오늘 고친 것이 iOS 에는 하나도 없었다. 1.0.4(build 18)는 **어제 15:15 KST** 에 올라간 것이라
가격 개편·코스 11종·청구 금액 버그 수정이 전부 빠져 있었다. **아이폰 사용자는 옛 가격으로 예약하고 있었다.**

- [x] `capacitor/package.json` 1.0.4 → **1.0.5**
- [x] Codemagic **build #19** 트리거 (`POST https://api.codemagic.io/builds`, 웹 모달은 확장에 가려 안 열린다)
- [x] 빌드 성공 · TestFlight 업로드까지 완료 (Publishing=success)
- [ ] **App Store 심사 제출** — ASC 세션이 만료돼 사장님 로그인이 필요하다.
      릴리즈 노트 초안은 `IOS_진행상황.md` 에 적어 뒀다.

빌드 파이프라인은 저장소 루트 `index.html` 을 그대로 `www/` 로 복사한다.
웹에 반영하면 앱에도 자동으로 들어가므로 따로 동기화할 일이 없다.

### W2. build 19 → 20 · 1.0.5 심사 제출 완료 (2026-09-06)

**build 19 를 올린 뒤 왁싱 예약이 막히는 것을 발견했다.** 내가 만든 회귀다.
`'왁싱 60분'` 을 내렸는데 앱이 `name.startsWith('왁싱')` 으로 서비스를 찾고 있었다.
활성 항목 중 '왁싱'으로 시작하는 이름이 하나도 없어 예약이 차단됐다.

- [x] 뷰티 서비스를 이름 **조각 포함**으로 모으게 고쳤다 (`BEAUTY_KEYS`).
      네일=네일·매니큐어·페디큐어 · 왁싱=왁싱 · 각질=각질·스크럽.
- [x] 홈뷰티도 **코스 화면을 거치게** 해서 부위를 직접 고르게 했다.
      예전에는 시간 선택으로 바로 넘어가 '왁싱 60분' 하나로 뭉뚱그려 팔렸다.
      이제 겨드랑이 250,000 ~ 풀 왁싱 1,200,000 을 골라서 예약한다.
- [x] `book()` 이 화면만 바꾸고 다시 그리지 않아 목록이 이전 상태로 남던 문제.
- [x] 홈뷰티를 본 뒤 마사지사를 고르면 **각질 목록이 그대로 남던** 문제 (종류 초기화).
- [x] build 20 재빌드 → 제출.

**검증** — 왁싱 7개·네일 4개·각질 2개가 부위별로 뜨고, 마사지(11종)로 왕복해도 목록이 정확히 바뀐다.

- [x] **1.0.5 (build 20) 심사 제출 완료 · `WAITING_FOR_REVIEW`** (최대 48시간)

## X. 구글 플레이 프로덕션 신청 (2026-09-06)

- [x] 세 조건 충족 확인 — 대시보드에 `check` 3개
- [x] **프로덕션 액세스 신청 접수 완료 (20:38)** · 검토 보통 7일 이내 · 결과는 메일
- 답변 내용은 `PLAY_STORE_진행상황.md` 참고. **사장님 확인 후 제출했다.**

⚠️ 앱 ID 를 헷갈리지 말 것 — massa `4975749612232381832` / 모아학원 `4973452451763223919`.
   처음에 모아학원 프로덕션 페이지를 열어 시간을 버렸다.
⚠️ Play Console 은 합성 이벤트로 버튼이 안 눌릴 때가 있다.
   `btn.click()` 이 안 먹으면 `scrollIntoView` → `focus()` → 좌표 포함 PointerEvent 순으로 보낸다.
   (실제로는 `pointerdown/mousedown/pointerup/mouseup/click` 을 좌표와 함께 dispatch 해서 열렸다)

## Y. 심사 대기 마사지사 확인 (2026-09-06)

사장님이 "심사 대기 6명이 누구냐"고 물으셨다. **6명 모두 실제 지원자가 아니었다.**

| 누구 | 정체 |
|---|---|
| Thanh hà ×4 | `demo@massa.app` 테스트 계정이 2026-08-29 **04:03~04:05, 2분 안에 4건** 제출. 사진 URL 4건 동일 |
| 무명 | 같은 demo 계정, 2026-07-12. 이름이 '무명', 사진·소개 없음 |
| Trang B. | 2026-07-07 시드. 사진이 `https://example.com/trang.jpg` — 존재하지 않는 주소 |

예약·정산·입금·리뷰 참조 0건. `provider_services` 14건만 붙어 있다.

### ★ 함께 드러난 것 — 승인된 21명 중 14명도 시드다
- 21명 **전원**이 기본 유니폼 이미지를 쓴다.
- 그중 **14명은 `profile_id`·`owner_id` 가 둘 다 없다.** 앱 계정과 이어져 있지 않다 = 실제 사람이 아니다.
- 앱은 이미 App Store 에 나가 있으므로 **고객이 이 14명을 보고 예약할 수 있다. 받을 사람이 없다.**
- 사장님 판단: **지금은 그대로 둔다** (오픈 전이라 화면이 비어 보이는 것보다 낫다).
  실제 마사지사가 채워지면 `is_active=false` 로 내리기로 했다.

### 조치한 것 — 중복 신청 방지
`demo` 계정 하나가 provider 를 5개 만들 수 있었다. 앱 화면만 고치면 우회되므로 DB 에서 막았다.

- 유니크 인덱스가 아니라 **트리거**를 썼다. 인덱스로 걸면 이미 있는 중복 5건 때문에 생성 자체가 실패한다.
  기존 데이터를 건드리지 않고 앞으로 들어오는 것만 막아야 했다.
- 반려(`rejected`)된 사람은 다시 신청할 수 있다. 대기·승인 상태만 셈에 넣는다.
- 계정 없이 넣는 시드 데이터는 그대로 통과시킨다.

**검증 (5가지 모두 통과)**
| 시험 | 결과 |
|---|---|
| 이미 신청이 있는 계정이 또 넣기 | 거부 |
| 신청이 없는 계정 | 통과 |
| 그 계정이 또 넣기 | 거부 |
| 반려 뒤 재신청 | 통과 |
| 계정 없는 시드 | 통과 |

시험 데이터 3건은 지웠다. 최종 27명(승인 21 · 대기 6) 그대로.

## Z. 마사지사 개인 가격 (2026-09-08)

기본가는 그대로 두고, 마사지사가 **본인 코스 가격을 올릴 수 있게** 한다.

**사장님이 정한 규칙**
| 항목 | 결정 |
|---|---|
| 범위 | 기본가 이상만. 상한은 기본가 × 배수 (기본 2배, 관리자가 조정) |
| 승인 | 관리자 승인 후 손님에게 보인다 |
| 단위 | 코스별 개별 |

**계획**
1. DB: `provider_services` 에 가격 칸 추가 + 검증·승인 RPC → verify: 시험 스크립트 8항목
2. DB: 예약 금액을 **서버에서 다시 계산**하는 트리거 → verify: 조작된 금액이 덮어써지는지
3. 앱: 고른 마사지사의 가격으로 화면 전체 교체 → verify: 브라우저에서 두 마사지사 비교
4. 콘솔(파트너): 내 가격 화면 → verify: 상한 초과가 막히는지
5. 콘솔(관리자): 가격 승인 화면 → verify: 승인 전/후 앱 표시 차이

### 진행
- [x] Z1. `scripts/provider_price.sql` — 새 표 `provider_price` + 함수 6개
- [x] Z2. 예약 금액 서버 재계산 트리거 `trg_freeze_booking_amount`
- [x] Z3. 시험 스크립트 `scripts/test_provider_price.sh` — 12항목 전부 통과
- [x] Z4. index.html — `priceOf()` 로 가격 출처 일원화
- [x] Z5. admin.html — 파트너 '내 가격'
- [x] Z6. admin.html — 관리자 '가격 승인'
- [x] Z7. 배포 (VPS + Vercel) · 브라우저 확인

### 검증 (DB 12항목)
| 시험 | 결과 |
|---|---|
| 기본가보다 낮게 | 거부 |
| 상한(2배)보다 높게 | 거부 |
| 남의 프로필 가격 변경 | 거부 |
| 정상 범위 → 대기로 들어감 | 통과 |
| 승인 전 손님 목록 노출 | 안 보임 |
| ★ 화면이 9,999,999₫ 를 보내도 | 500,000₫ 로 덮어씀 |
| 관리자 승인 | 적용가 반영 |
| ★ 승인 뒤 1₫ 를 보내도 | 600,000₫ 로 덮어씀 |
| 승인 뒤 공개 목록 | 보임 |
| 다른 마사지사 | 기본가 그대로 |
| 반려 | 대기 지워짐 · 사유 남음 · 적용가 유지 |
| 비우기 | 기본가로 복귀 |

### 검증 (브라우저, massa.moahagwon.com)
Mai H. 아로마테라피 60분에 650,000₫ 을 승인해 두고 확인했다.

| 화면 | Mai H. | 다른 마사지사 |
|---|---|---|
| 코스 선택 (아로마 60분) | 650,000₫ | 500,000₫ |
| 코스 선택 (스웨디시 60분) | 500,000₫ | 500,000₫ |
| 프로필 '내 서비스' | 650,000₫ | — |

확인 뒤 시험 데이터는 지웠다 (`provider_price` 0건).

### 곁다리로 고친 것 — 파트너 메뉴가 아무에게도 안 보이고 있었다
`loadAccount()` 의 파트너 판정이 `profile_id` 만 봤다. 그런데 **승인된 마사지사 21명 중
`profile_id` 를 가진 사람은 0명이고 7명 전부 `owner_id` 로 붙어 있다.**
`myProviders()` 는 둘 다 보는데 판정만 안 봐서, 예약 관리·정산·채팅·GPS 등
**파트너 메뉴 전체가 통째로 숨겨져 있었다.** 판정도 둘 다 보게 맞췄다.

운영 콘솔의 `amount_vnd: svc?.price_vnd || 850000` 도 같이 없앴다.
고객 앱에서는 진작 고쳤는데 콘솔에만 남아 있었다.

### 남겨 둔 것 (고치지 않음, 확인 필요)
- 프로필 화면의 '내 서비스' 는 `is_active` 를 거르지 않는다. 내린 코스가 보이고, 그걸 고르면
  `currentService()` 가 못 찾아 예약이 막힌다.
- 스웨디시·타이 **120분은 `provider_services` 에 안 붙어 있다.** 프로필 화면 칩에 안 뜬다.
  (코스 선택 화면에서는 뜨므로 예약 자체는 된다)
- **심사 대기가 6명 → 7명으로 늘었다.** 새 신청 `Kun` (계정 b2ae7bb8). 앞의 6명과 달리
  demo 계정이 아니라 별도 계정이다.

## Z-2. 가격 일괄 승인 · 애플 계정 합치기 (2026-09-08)

### 일괄 승인
가격 승인 화면 맨 위에 **전체 선택 · N건 선택 · 선택 승인** 막대를 붙였다.
반려는 사유가 건마다 달라야 하므로 지금처럼 한 건씩 둔다.

한 건이 막혀도 나머지는 승인한다. 전부 되돌리면 관리자가 어느 게 문제인지 모른 채
처음부터 다시 골라야 한다. 막힌 건은 이름·코스·사유를 그대로 보여 준다.

| 시험 | 결과 |
|---|---|
| 관리자가 아닌 계정 | 거부 |
| 정상 2건 + 상한 초과 1건을 한 번에 | 2건 승인 · 1건은 사유와 함께 대기 유지 |
| 승인된 2건이 손님 목록에 | 나옴 |
| 빈 목록 | 거부 |

### 애플 계정을 사장님 구글 계정으로 합침
앱에서 애플로 로그인하면 GoTrue 가 **별도 auth 사용자**를 만든다.
그래서 `karmadc77@icloud.com` 은 `profiles` 행조차 없었고 운영 콘솔이 열리지 않았다.

- 합치기 전에 `auth` 스키마를 통째로 백업했다 (`/root/auth_backup_20260908_205608.sql`)
- `auth.users`·`profiles` 를 가리키는 **외래키 14곳을 코드로 훑어** 애플 계정에 딸린 데이터가
  0건임을 확인한 뒤에만 진행하도록 했다 (한 건이라도 있으면 중단)
- 데이터를 옮긴 게 아니라 **신원(identity)만** 구글 계정으로 옮기고 빈 계정을 지웠다

결과: `karmadc070@gmail.com` = `admin` · 로그인 수단 **apple · email · google**.
이제 아이폰에서 애플로 로그인해도 같은 계정이고, 관리자로 보인다.

### 심사 대기 명단 (7명)
| 이름 | 계정 | 신청 | 예약·리뷰·채팅 |
|---|---|---|---|
| Trang B. | demo@massa.app | 07-08 | 0 |
| 무명 | demo@massa.app | 07-12 | 0 |
| Thanh hà ×4 | demo@massa.app | 08-29 | 0 |
| **Kun** | **trinh1111ttt@gmail.com** | **09-08 03:17** | 0 |

demo 6건은 시험 데이터다. **Kun 만 별도 계정에서 온 새 신청**이다.

## Z-3. 가입 필수 항목 · Kun 후속 (2026-09-08)

### 필수 항목(*) 검사
예전에는 **아무것도 안 채워도 신청이 통과했다.** 그래서 '무명' 신청과 서류 없는 승인자가 쌓였다.

- 라벨 9곳에 빨간 `*` 를 붙였다. 라벨 글자와 분리된 `<span>` 이라 **번역이 깨지지 않는다**
  (베트남어에서도 `Tên hoạt động (thật)*` 로 확인)
- 빈 항목을 **한 번에 모아서** 알려 준다. 하나씩 튕겨 내면 몇 번을 더 눌러야 하는지 모른다
- 빈 칸은 빨갛게 칠하고 첫 번째 칸으로 화면을 옮긴다
- 필수: 활동명 · 대표사진 · 구분 · 제공서비스 · 활동지역 · 연락처 · 은행명 · 계좌번호 · 신분증 앞뒤
  (샵이면 사업자등록증 추가) / 선택: 소개 · 자격증 · 사진 2~5
- `display_name: ... || '무명'` 대체값을 없앴다. 이 값으로 들어온 신청이 실제로 있었다

**검증 (가짜 DOM 으로 함수를 직접 돌림, 6/6 통과)**
빈 폼 → 9가지 · 구분 해제 → 구분 포함 · 신분증만 누락 → 2가지 · 완전한 폼 → 0가지 ·
샵인데 사업자등록증 없음 → 1가지 · 이름이 공백뿐 → 걸림

### 승인 뒤에도 서류를 넣을 수 있게 함
**승인되면 "✅ 승인 완료" 문구만 뜨고 신분증·계좌를 넣을 곳이 아예 없었다.**
Kun 에게 "서류 넣어 주세요" 라고 말할 데가 없어서 발견했다.
승인 상태를 **유지한 채** 정보·서류만 저장하는 버튼을 넣었다.
심사로 되돌리면 고객 목록에서 사라져 영업을 못 하게 되므로 상태는 건드리지 않는다.

### 빠져 있던 코스 연결
- Kun: `provider_services` 0건 → 본인이 고른 아로마·스웨디시·타이 × 60/90/120 = **9건**
- 스웨디시 120분 · 타이 마사지 120분: **아무에게도 안 붙어 있었다** → 같은 종류 60·90분을
  하는 사람에게만 붙였다 (안 하던 종류를 새로 떠안기지 않는다). 각 12명
- 결과: 코스가 0건인 승인 마사지사 **0명**

### Kun 안내 발송
채팅 3통(베트남어) + 앱 알림 1건. 첫 줄에 **운영팀임을 밝혀** 손님으로 오해하지 않게 했다.
**계좌번호·신분증을 채팅으로 보내지 말라고 명시**했다. 앱 안에서 넣으면 비공개로 저장된다.

### 전체 점검 결과 (2026-09-08)
| 항목 | 상태 |
|---|---|
| 컨테이너 massa-* 11개 | 전부 healthy |
| 디스크 / 메모리 | 8% 사용 · 7.2Gi 여유 |
| 외부 접속 6곳 | 전부 200 (www 는 301 정상) |
| 인증서 | 11/27~12/4 만료 |
| RLS 꺼진 public 표 | **0개** |
| 예약 금액·수수료 트리거 | 둘 다 살아 있음 |
| 백업 | `/root/backups` 에 매일 03:10, 7일 보관, 로그 전부 OK |
| 관리자 | `karmadc070@gmail.com` = admin · apple·email·google 합쳐짐 |

**iOS 1.0.5 = `PENDING_DEVELOPER_RELEASE`** — 심사 통과. 출시 버튼만 누르면 된다.

## Z-4. 사진 위 로고 패치 제거 (2026-09-10)

사장님이 `사진에 massa가 얼굴을 가리는 것 같다` 고 하셨다. 실제로 그랬다.

`.logopatch` 는 사진 위에 CSS 로 얹던 베이지색 라벨이다.
`left:50%; bottom:30%` 라 **120px 프로필 사진에서는 사람 몸통을 상자처럼 덮었다.**
코드 주석에는 `첫 사진 가슴 위치` 라고 돼 있었지만, 정사각형으로 잘린 작은 썸네일에서는
가슴이 아니라 턱 바로 아래였다. 큰 화면 기준으로 잡은 위치가 작은 화면에서 무너진 경우다.

- CSS 규칙, 업로드 그리드의 span, 프로필 화면의 span 을 모두 지웠다
- `renderApPhotos` 의 패치 토글도 같이 지웠다 (내 변경으로 고아가 된 코드)
- 패치가 `AI 변환 중…` 표시도 겸하고 있었다. 사진을 가리지 않게 **그리드 아래 글씨**(`#apAiMsg`)로 옮겼다

**유니폼 로고 자체는 그대로다.** AI 변환(`UNIFORM_PROMPT`)이 이미지 안에
왼쪽 가슴에 작게 자수로 그려 넣는다. 화면에서 덧그릴 필요가 없었던 것뿐이다.

확인: Kun 프로필에서 상자가 사라지고 얼굴·유니폼이 그대로 보인다. 배포 파일 `logopatch` 0건.

`마싸_고객앱_와이어프레임.html` 에도 같은 코드가 있으나 배포되지 않는 옛 산출물이라 두었다.

## Z-5. 사진 확대 · 실제 거리 (2026-09-10)

### ★ 거리가 지어낸 값이었다
`providerCard` 에 `const dist = (0.6 + idx * 0.6).toFixed(1)` 이 있었다.
**목록 순번에 0.6 을 곱한 숫자**를 `km` 로 붙여 보여 줬다. 프로필의 `◎ 100m` 도 그냥 박아 둔 글자였다.
손님은 그 숫자를 보고 가까운 사람을 고른다. 지어낸 값을 근거로 고르게 두면 안 된다.

- `haversineKm` 으로 실제 계산. 마사지사 21/22 명이 좌표를 가지고 있다 (Kun 만 없다)
- **거리를 모르면 아예 말하지 않는다.** 권역만 표시하거나 비워 둔다
- 위치는 앱 켜자마자 묻지 않는다. 목록 위에 **왜 필요한지 적은 줄**을 두고 누를 때만 요청한다.
  이유도 모른 채 거절당하면 다시 물을 방법이 없다
- 이미 허용해 둔 사람에게는 `navigator.permissions` 로 확인해 조용히 쓴다
- 위치를 얻으면 목록을 다시 그리지 않고 `.tdist` 글자만 갈아 끼운다.
  통째로 다시 그리면 스크롤 위치와 열어 둔 필터가 날아간다
- iOS 는 `NSLocationWhenInUseUsageDescription` 이 이미 codemagic.yaml 에 있어 그대로 동작한다

**검산 (10/10 통과)** — 실제 DB 좌표로 미딩↔바딘 3.98km, 미딩↔호안끼엠 7.63km,
바딘↔호안끼엠 4.01km, 위도 1도 111.19km. 1km 미만은 m 로 표시.

### 사진 확대 보기
프로필 사진이 120px 라 얼굴도 유니폼도 안 보였다. 눌러서 크게 볼 수 있게 했다.

- 여러 장이면 좌우 화살표 + 점 표시. ESC·배경 클릭·✕ 로 닫는다
- 사진이 2장 이상이면 썸네일 모서리에 `📷 N` 을 붙여 더 있다는 걸 알린다
- 확인: Kun(3장) 에서 확대·다음 사진 이동 모두 동작

### 남겨 둔 것
- `내 근처` 칩은 아직 정렬을 하지 않는다 (`pick(this)` 로 선택 표시만 한다).
  이제 실제 거리가 있으니 정렬을 붙일 수 있다. 목록 순서가 바뀌는 건 사장님 판단이라 두었다
- Kun 은 좌표가 없어 거리·권역이 모두 비어 있다. 파트너 화면의 `근무 위치(GPS)` 에서 한 번 켜면 채워진다
- AI 유니폼 변환은 **1번 사진에만** 적용된다. Kun 의 2·3번은 사복 사진이다

## Z-6. 호텔 입력 → 구글 지도 검색 (2026-09-11)

호텔 칸은 그냥 빈 입력창이었다. 라벨만 `호텔 검색 (Google Places)` 라고 적혀 있고
실제로는 아무것도 연결돼 있지 않았다. 화면이 하지 않는 일을 약속하고 있었다.

### 만든 것
| 부분 | 내용 |
|---|---|
| `functions/places-search` | 구글 Places 를 중계하는 Edge Function |
| `bookings.lat/lng/place_id` | 방문 좌표 저장 (범위 검사 붙임) |
| 숙소 검색 시트 | 눌러서 검색 · 350ms 지연 · 8건까지 · 직접 입력 대비책 |
| 마사지사 `🧭 길찾기` | 좌표가 있으면 그 지점으로, 없으면 이름으로 검색해서 연다 |

### 지킨 것
**키를 앱에 넣지 않는다.** Capacitor 앱은 HTTP 리퍼러 제한이 안 걸려서,
앱 파일에 키를 두면 누구나 꺼내 쓸 수 있고 그 요금은 사장님이 낸다.
키는 Edge Function 환경변수에만 두고 `vps-apply-maps-key.sh` 가 넣은 뒤 `shred` 로 지운다.
값은 화면에 찍지 않고 길이만 보여 준다.

**검색이 죽어도 예약은 계속된다.** 함수가 실패하면 "직접 입력해 주세요" 로 넘어간다.
지도가 예약을 막는 단일 장애점이 되면 안 된다.

**요금을 줄이는 장치.** 검색 한 번에 세션 토큰 하나를 쓰고(구글이 한 건으로 묶는다),
글자를 칠 때마다가 아니라 350ms 멈췄을 때만 보내고, 상세 조회는 필요한 4개 칸만 받는다.
칸을 늘리면 요금 등급이 올라간다.

**이름을 손으로 고치면 좌표를 버린다.** 이름과 좌표가 어긋난 채 길찾기가 열리면
마사지사가 엉뚱한 곳으로 간다.

**위치 선택자를 id 로 바꿨다.** `#loc input.tx` 를 순서로 집던 코드가 3곳 있었다.
칸을 하나만 넣어도 호수와 호텔이 뒤바뀐다.

### 남은 것 — 사장님이 해주셔야 함
구글 클라우드에서 **Places API (New)** 키를 만들어 `/root/maps.key` 에 한 줄로 저장 →
`sh /root/vps-apply-maps-key.sh` 실행. 그 전까지는 직접 입력으로 동작한다.
월 10,000건까지 무료라 현재 규모(설치 41)에서는 요금이 발생하지 않는다.

## Z-7. 구글 플레이 프로덕션 출시 제출 (2026-09-11)

**프로덕션 액세스가 승인됐다.** "축하합니다. 앱에 Google Play 프로덕션 액세스 권한이 부여되었습니다."
(9/6 신청 → 9/11 승인, 5일 걸림)

### 새로 빌드하지 않았다
안드로이드는 `massa-seven.vercel.app` 을 여는 TWA 라, 번들은 껍데기일 뿐이고
앱 내용은 Vercel 배포로 이미 최신이다. 비공개 테스트에서 심사를 통과한
**번들 1 (1.0.0.0)** 을 라이브러리에서 그대로 가져다 썼다.

### 걸렸던 것
`이 트랙에 선택된 국가 또는 지역이 없습니다` — 프로덕션 트랙은 국가를 따로 정해야 한다.
비공개 테스트 때 정한 것이 넘어오지 않는다.
**베트남 + 대한민국** 으로 잡았다. iOS 가 이미 두 나라에 올라가 있어 스토어를 맞췄다.
(하노이 현지 손님 + 출장·여행 전에 미리 까는 한국 손님)

### 제출 내용
| 항목 | 값 |
|---|---|
| 번들 | 1 (1.0.0.0) · 640KB |
| 국가 | 베트남, 대한민국 |
| 출시 범위 | 전체 출시 (100%) |
| 출시 노트 | ko-KR 1개 언어 |

구글 사전 검사(최대 14분)를 통과하면 자동으로 심사에 들어간다. 심사는 보통 7일 이내.

### Play Console 자동화 메모
- `/tracks/production/create` 로 직접 가면 `예기치 않은 오류` 가 난다. 화면에서 버튼을 눌러 SPA 로 이동할 것
- 버튼은 좌표 클릭이 어긋난다. `pointerdown/mousedown/pointerup/mouseup/click` 을 요소에 dispatch 할 것
- `mat-checkbox` 는 내부에 `input` 이 없다. 선택 여부는 `aria-checked` 로 읽어야 한다
- 브라우저 창이 좁으면 Play Console 이 내용을 안 그린다. 1440x900 으로 넓히고 볼 것

### 곁다리로 확인된 것
출시 노트 언어가 **ko-KR 하나뿐**이다. 베트남 스토어에 내면서 베트남어 등록정보가 없다.
App Store 도 `languageCodesISO2A: ["EN"]` 하나뿐이다. 양쪽 다 현지어 등록정보가 빠져 있다.

## Z-8. 순서대로 진행 — A · B (2026-09-11)

### A. 프로필이 내린 코스를 거르지 않던 문제 — 완료
`openProfile` 의 `provider_services` 조회에 `is_active` 필터가 없었다.
내린 코스가 프로필에 보이고, 손님이 그걸 고르면 `currentService()` 가 활성 목록에서만 찾으므로
예약이 막혔다. "코스를 다시 골라 주세요" 만 뜨고 무엇을 골라야 하는지는 알 수 없다.

**얼마나 심했나**: 마사지사에게 붙은 연결 691건 중 **176건(25.5%)이 내린 코스**였다.
프로필에 보이는 코스 4개 중 1개는 고르면 막히는 코스였다.
(아로마 마사지 · 어깨·목 마사지 · 왁싱 60분 · 태국식 마사지 · 풋 테라피 · 헤드 테라피 — 각 11명)

고친 뒤 프로필이 비는 마사지사는 **0명**. 브라우저에서 Mai H. 프로필을 열어
활성 11종만 뜨고 내린 코스는 하나도 안 섞인 것을 확인했다.

### B. 베트남어 스토어 등록정보 — 작성·저장 완료, 제출은 대기
하노이가 주 무대인데 Play 는 ko-KR 하나, App Store 는 EN 하나뿐이었다. 현지 검색에 안 잡힌다.

Play 에 **베트남어(vi)** 등록정보를 새로 썼다. 앱 이름 23/30 · 간단한 설명 79/80 ·
자세한 설명 2616/4000. 원문은 `store-assets/vi-VN-등록정보.md` 에 남겼다.
**초안 변경사항** 으로 저장됐다. 지금 프로덕션 출시가 심사 중이라 그 심사가 끝난 뒤 함께 제출된다.

### ★ 내가 만든 미완 — 빈 영어 등록정보
언어를 추가할 때 **영어(en-US)를 같이 켜 놓고 내용을 안 채웠다.**
그래서 `일부 언어에 오류가 있습니다` 가 뜨고 `저장` 이 막힌다(임시보관함 저장은 된다).
**다음 제출 전에 영어를 채우거나 빼야 한다.**

`번역 관리` 드롭다운이 이 환경에서 열리지 않아 이번에 정리하지 못했다.
좌표 클릭·합성 이벤트 모두 실패. 사람이 직접 하면 3번 클릭이면 된다:
`스토어 등록정보 → 번역 관리 → 언어 선택 → 영어(미국) 체크 해제 → 적용`

### Play Console 자동화 메모 (추가)
- 뷰포트를 1440x900 으로 `resize_window` 하면 **스크린샷 좌표계(800x500)와 어긋나** 클릭이 빗나간다.
  글을 읽을 때만 넓히고, 클릭할 때는 `preset: desktop` 으로 되돌릴 것
- `publishing-overview` 도 URL 직접 이동이 안 된다. 대시보드로 간 뒤 사이드바를 눌러야 한다
- 폼 값은 `Object.getOwnPropertyDescriptor(proto,'value').set` 로 넣고
  `input`·`change`·`blur` 를 순서대로 dispatch 하면 Angular 가 받는다 (글자 수 표시로 확인됨)

### B 마무리 — 영어 등록정보까지 채움 (2026-09-11)

빈 채로 켜 두었던 **영어(en-US)** 를 채웠다. 앱 이름 23/30 · 간단한 설명 78/80 · 자세한 설명 2721/4000.
`일부 언어에 오류가 있습니다` 가 사라지고 **정식 저장(임시보관함이 아님)** 이 됐다.
`초안 변경사항` 배지도 없어졌다 — 검토 대기 변경으로 올라갔다는 뜻이다.

새로고침 후 다시 확인: 베트남어 23/79/2616 그대로 남아 있다.

**결과: Play 등록정보가 한국어 · 베트남어 · 영어 3개 언어.**
지금 프로덕션 출시 심사가 진행 중이라, 등록정보는 그 심사가 끝난 뒤 함께 제출된다.

#### 언어 드롭다운을 여는 법 (막혔다가 찾은 것)
`번역 관리` 옆의 **이름표 없는 button** 이 언어 선택기다.
`read_page filter:interactive` 로 `button [ref_N]` (라벨 없음) 을 찾아 **ref 로 클릭**하면 열린다.
좌표 클릭은 뷰포트를 `resize_window` 로 넓혀 둔 상태에서 스크린샷 좌표계와 어긋나 계속 빗나갔다.
**Play Console 을 클릭할 때는 뷰포트를 `preset: desktop` 으로 두고 ref 로 클릭할 것.**

#### 남은 것
- 원어민 검수 (뜻은 맞지만 어투는 손볼 여지가 있다)
- App Store 쪽은 아직 EN 하나뿐이다. 베트남어 현지화를 따로 넣어야 한다

## Z-9. 백업 암호화 — 서버 밖으로 보낼 준비 (2026-09-11)

백업은 매일 잘 돌고 있었지만 **같은 서버 안에만** 있었다. 서버가 죽으면 백업도 같이 죽는다.
그렇다고 그대로 밖에 내보낼 수도 없다 — 덤프 안에 이런 게 들어 있다:

`profiles(full_name, phone)` · `provider_kyc(contact, bank_name, bank_account, id_front_url, id_back_url)`

### 왜 비대칭(공개키)인가
암호 하나로 잠그는 방식은 **그 암호가 서버에 있어야** 매일 자동으로 잠글 수 있다.
서버가 털리면 백업도 같이 털린다. 백업을 밖에 두는 의미가 없어진다.

그래서 열쇠를 둘로 나눴다.
- **잠그는 열쇠(공개키)** — 서버에 둔다. 이것만으로는 못 연다
- **여는 열쇠(개인키)** — 사장님 PC `C:\Users\user\massa-backup-key` 에만 있다

지문: `DACC 12ED 0A78 9A7A 66B0  E766 7FF1 0971 2623 DD2F`

### 검증 — 다섯 가지를 실제로 확인했다
| 시험 | 결과 |
|---|---|
| 서버에서 잠갔다 풀기 (개인키 있을 때) | 원본과 한 바이트도 다르지 않음 |
| 서버에서 개인키 삭제 후 — 잠그기 | 됨 |
| 서버에서 개인키 삭제 후 — **풀기** | **안 됨** (이게 핵심) |
| ★ 사장님 PC 개인키로 풀기 | sha256 일치 |
| 푼 파일이 진짜 덤프인가 | pg_dump · 표 29개 |

마지막 두 개가 제일 중요하다. 서버가 잠근 파일이 **다른 기계에서 열려야** 백업이다.
안 열리면 그냥 파일 더미다.

### 지금 상태
- `/root/backups` — 평문. 서버 안에만 둔다 (매일 03:10)
- `/root/backups/enc` — 잠긴 것. **밖으로 나갈 수 있는 건 이것뿐** (매일 03:25)
- 35개 잠금 완료 · enc 에 평문 섞임 0건 · 7일 보관

### 남은 것 — 사장님이 해주셔야 함
**Cloudflare 대시보드에서 R2 를 켜 주세요.** (Cloudflare 계정은 이미 붙어 있고,
MCP 로 확인하니 `Please enable R2 through the Cloudflare Dashboard` 가 나온다.
월 10GB 까지 무료, massa 잠긴 백업은 하루 90KB 라 요금이 안 난다)

켜 주시면 제가 버킷을 만들고, R2 토큰만 `/root/r2.key` 에 넣어 주시면
업로드까지 자동으로 붙이겠습니다. 값은 제가 읽지 않습니다.

### ⚠ 개인키를 잃으면 백업을 영영 못 엽니다
`C:\Users\user\massa-backup-key\massa-backup-private.asc` (3,429 bytes)
USB 나 다른 PC 에 한 부 더 복사해 두세요. 이 파일이 없으면 잠긴 백업은 열 수 없습니다.
저장소에는 들어가지 않도록 `.gitignore` 에 넣어 두었습니다.

### D 마무리 — R2 업로드까지 (2026-09-11)

Cloudflare R2 를 켜고 버킷 `massa-db-backup` (ENAM) 을 만들었다. rclone v1.75.1 설치.

#### 막혔던 것 — 토큰 권한
처음 만든 토큰이 **Object Read only** 였다. 읽기는 되고 쓰기만 403 이 났다.
`rclone ls` 는 통과하는데 `PutObject` 만 막히는 걸 보고 권한 문제로 좁혔다.
(Access Key·Secret·엔드포인트·버킷 이름이 다 맞다는 뜻이기도 했다)
대시보드에서 **Object Read & Write** 로 바꾸니 바로 통과. 키 값은 그대로 써서 재전송이 필요 없었다.

#### ★ 재난 복구 전체 사슬 검증
서버가 아예 없다고 치고, **R2 에 있는 파일만으로** 복구되는지 확인했다.

| 단계 | 결과 |
|---|---|
| R2 에서 내려받기 | 90,055 bytes |
| 사장님 PC 개인키로 풀기 | 89,420 bytes |
| 서버 평문 덤프와 sha256 대조 | `f738a5f7…` **일치** |
| 실제 복구 가능한 덤프인가 | pg_dump · 표 29개 (bookings·profiles·app_settings…) |

**중간에 한 번 틀렸다.** 처음엔 *암호화 파일*의 해시를 *푼 파일*과 비교해 "실패" 가 떴다.
비교 대상이 잘못된 것이지 백업은 멀쩡했다.
GPG 는 매번 다른 세션 키를 쓰므로 `.gpg` 해시는 잠글 때마다 달라진다.
**푼 파일은 평문 덤프의 해시와 비교해야 한다.**

#### 최종 구성
| 위치 | 내용 | 보관 |
|---|---|---|
| `/root/backups` | 평문 | 7일 · 서버 밖으로 안 나감 |
| `/root/backups/enc` | 잠긴 것 | 7일 |
| `r2:massa-db-backup/enc` | 잠긴 것 | **30일** |

크론: 03:10 백업 → 03:25 잠금 → 03:40 업로드

**`sync` 가 아니라 `copy` 를 쓴다.** sync 는 서버에서 지워진 것을 원격에서도 지운다.
서버가 털려 백업이 삭제되면 원격 사본까지 같이 사라진다 — 백업을 밖에 두는 이유가 없어진다.
서버는 7일, 원격은 30일이라 서버에서 사라져도 한 달은 되돌릴 수 있다.

업로드 직전에 `enc` 에 `.gpg` 아닌 파일이 있으면 **중단한다.** 평문 유출을 막는 마지막 빗장이다.

#### PowerShell 함정 (또 당함)
- `scp` 대상에 콜론(`root@host:/path`)이 있으면 **반드시 따옴표**로 감싸야 한다
- `.ps1` 에 한글이 들어가면 Windows PowerShell 이 BOM 없는 UTF-8 을 잘못 읽어
  **문자열 파싱이 깨진다**. 스크립트는 ASCII 로만 쓸 것


---

## V. Vercel 탈출 — 앱을 app.massaviet.com 으로 옮기기

목표: 앱 본체가 `massa-seven.vercel.app` 이 아니라 **사장님 도메인·사장님 서버**에서 돌게 한다.
massaviet.com(소개 사이트)은 이미 VPS 에 있다. 앱만 남았다.

원칙: **한 번에 끊지 않는다.** 새 주소를 먼저 띄워 검증하고, 옛 주소는 한동안 같이 살려 둔다.
이미 설치된 안드로이드 앱은 업데이트를 받기 전까지 옛 주소를 본다. 지금 끄면 그 사람들 앱이 죽는다.

### V1. 새 주소 띄우기 (되돌릴 수 있는 단계)
- [x] V1-1. DNS `app.massaviet.com` A `141.164.46.88` + AAAA 추가 (Cloudflare, **DNS only**) — 2026-09-15 완료
- [x] V1-2. Caddy 에 `app.massaviet.com` 블록 추가, 루트 `/srv/massa-app` — 인증서 자동 발급 확인
- [x] V1-3. `scripts/vps-deploy-app.sh` 작성·실행 — 파일 48개 배포, 9개 경로 전부 200
- [x] V1-4. **Vercel 은 그대로 둔다.** 두 주소가 같이 산다.

#### V1 검증 결과 (2026-09-15)
- assetlinks 지문 2개 그대로 나옴 → TWA 전환 준비됨
- `.env`·`signing.keystore`·`scripts/*` 전부 404 → 비밀 유출 없음
- 실제 앱 동작: 테라피스트 목록이 DB 에서 정상 로드 (Mai H.·Linh N. … 평점·권역까지)
- `config.js` 404 는 **Vercel 에서도 똑같이 404** 다. gitignore 된 로컬 파일이라 원래 그렇다. 이전 탓 아님.

### V2. 로그인이 새 주소에서 되게 하기 — 2026-09-15 완료
- [x] V2-1. GoTrue `ADDITIONAL_REDIRECT_URLS` 에 `app.massaviet.com/*` · `/**` 추가, auth 재기동
      → 컨테이너 안 `GOTRUE_URI_ALLOW_LIST` 에 들어간 것 확인. 기존 주소는 하나도 빼지 않음
- [x] V2-2~4. **구글·카카오·애플 콘솔은 손댈 필요가 없었다.** 앞선 계획이 틀렸다.
      공급자에 보내는 `redirect_uri` 는 앱 주소가 아니라 **GoTrue 콜백**
      `https://api.moahagwon.com/auth/v1/callback` 로 고정이다. 셋 다 실제로 확인했다.
      앱 도메인이 바뀌어도 공급자 콘솔은 그대로다. 카카오 콘솔을 열 일도 없었다.
- [x] V2-5. `RESET_REDIRECT`·`PAY_START` 가 `massa.moahagwon.com` 을 가리키고 있었다 →
      `app.massaviet.com` 으로 교체. **학원 도메인이 massa 손님 메일에 뜨면 안 된다.**
      → verify: 배포본 index.html 의 massa.moahagwon.com 0건, 결제·재설정 4개 페이지 200

#### 남겨 둔 것
- `api.moahagwon.com` (API 엔드포인트) 는 그대로 둔다. 손님 눈에 보이지 않고,
  바꾸려면 새 vhost·인증서·전 클라이언트 교체가 따라온다. 별건이다.
- `admin.html` 의 RESET_REDIRECT 는 그대로 둔다. 운영은 admin.moahagwon.com 에서 한다.

### V3. 새 주소 전체 검증 (여기서 막히면 V4 로 가지 않는다)

#### 자격증명 없이 확인한 것 — 2026-09-15 전부 통과
- [x] 화면 렌더링: `/`·`admin.html`·`reset.html`·`delete-account.html`·`privacy`·`terms` 정상
- [x] **콘솔 에러 0건** (새 탭 기준). `config.js` 404 는 배포 때 빈 파일을 놓아 없앴다
- [x] 서비스워커 등록·활성 확인, 앱이 `api.moahagwon.com` 을 바라봄
- [x] **새 출처에서 CORS 통과** — providers·services·stores 전부 200
- [x] **Edge Function 도 새 출처에서 동작** — places-search 200, 후보 4건
- [x] V3-5. 지도 검색 정상 (토큰 없는 요청은 여전히 401)
- [x] 사본 비교: `/srv/massa-web`(massa.moahagwon.com) 은 9/11 자, 기능은 동일.
      차이는 RESET_REDIRECT 한 줄뿐이라 낡아서 위험한 상태는 아니다

#### 리디렉트 허용목록 — 비밀번호 없이 동작으로 증명함 (2026-09-15)
첫 시도는 **실패한 방법**이라 남겨 둔다. `/auth/v1/recover` 에 redirect_to 를 넣어 봤더니
허용목록 밖 주소까지 전부 200 이 나왔다. GoTrue 는 **요청받을 때가 아니라 돌려보낼 때** 판정한다.

판정이 실제로 일어나는 `/auth/v1/verify` 에 걸었더니 갈렸다. 토큰은 엉터리로 줘도 된다 —
보는 것은 Location 헤더지 인증 성공 여부가 아니다.

| 넣은 redirect_to | 돌려보낸 곳 | 판정 |
|---|---|---|
| app.massaviet.com/reset.html | **app.massaviet.com/reset.html** | 허용됨 |
| massa.moahagwon.com/reset.html | massa.moahagwon.com/reset.html | 허용됨 |
| evil.example.com/steal | massa.moahagwon.com (SITE_URL) | **튕김** |

허용목록 밖은 SITE_URL 로 튕기고 새 주소는 그대로 돌아온다. 이게 이 이전에서
조용히 깨질 수 있던 유일한 지점이었고, 이제 막혔다.

#### 사장님이 해 주셔야 하는 것 (비밀번호가 필요해 대신 못 한다)
- [ ] V3-1. `https://app.massaviet.com` 에서 **로그인 1회** → 세션이 잡히는지
      (위 증명으로 위험은 거의 사라졌다. 남은 건 자격증명 교환 한 단계뿐이다)
- [ ] V3-2. 예약 한 건 생성 → 금액이 서버값으로 덮이는지
- [ ] V3-3. 채팅 메시지 왕복
- [ ] V3-4. 관리자 화면 진입 (is_admin 통과)

로그인만 되면 나머지는 기존 기능이라 같이 따라온다. **로그인이 이 이전의 유일한 미확인 지점이다.**

### V4. 안드로이드 갈아타기 (심사 1회)

> **지금은 시작할 수 없다.** 프로덕션 1.0.0.0 이 2026-09-11 제출 이후 아직 **검토 중**이다.
> 검토 중에 새 버전을 올리면 진행 중인 심사가 엎어지거나 늘어진다.
> **Play 프로덕션이 공개된 뒤에** V4 를 시작한다. 매일 10시 예약 작업이 그 시점을 잡아 준다.

- [ ] V4-1. TWA origin 을 app.massaviet.com 으로 바꿔 재빌드 (Bubblewrap)
- [ ] V4-2. `app.massaviet.com/.well-known/assetlinks.json` 이 **기존 지문 2개 그대로** 나오는지
      → verify: 지문이 다르면 앱이 주소창을 띄운다 (TWA 가 깨진 것)
- [ ] V4-3. 새 AAB 를 Play 프로덕션에 올리고 심사 → verify: 업데이트 후 주소창이 안 뜸

### V5. iOS 1.0.7 — 2026-09-15 심사 제출 완료 (build 22)
- [x] V5-1. `RESET_REDIRECT`·`PAY_START` 를 app.massaviet.com 으로 교체한 채로 빌드
- [x] V5-2. 마케팅 URL → `https://massaviet.com`, 지원 URL → `https://massaviet.com/contact.html`
- [x] V5-3. **부제 채움** (비어 있었다) — "하노이 집·호텔로 찾아오는 방문 예약" 20자
- [x] V5-4. 키워드 12개 → 17개로 교체. 이름·부제에 있는 낱말은 뺐다 (자동 색인됨)
- [x] V5-5. **번들 언어 선언** — codemagic.yaml 에 CFBundleLocalizations(ko·vi·en·ja·zh) +
      CFBundleDevelopmentRegion=ko 단계 추가. 제품 페이지 '언어' 가 영어 하나로 뜨던 문제
- [x] V5-6. **출시 방식 = 자동**. 1.0.6 때는 심사 중이라 못 바꿨다. 이번엔 제출 전에 바꿨다

주의 — `languageCodesISO2A: ["EN"]` 은 **번들이 선언한 언어**지 등록정보 언어가 아니다.
등록정보 기본 언어는 처음부터 한국어였다. 이걸 헷갈려서 한 번 잘못 짚었다.

### V6. 옛 주소 정리 (몇 달 뒤)

> ### ★ Vercel 을 끄기 전에 반드시 먼저 할 것 — 스토어 법적 URL 교체
> 구글 플레이는 **개인정보처리방침 URL 이 반드시 열려야** 한다고 요구한다.
> 지금 두 스토어에 등록된 주소가 전부 vercel 이다. 그대로 Vercel 을 끄면
> 그 주소가 404 가 되고 **앱이 제재를 받는다.** 순서를 틀리면 안 된다.
>
> | 항목 | 지금 (옛 주소) | 바꿀 값 | 상태 |
> |---|---|---|---|
> | 개인정보처리방침 | massa-seven.vercel.app/privacy.html | `https://massaviet.com/privacy.html` | 200 확인 |
> | 이용약관 | massa-seven.vercel.app/terms.html | `https://massaviet.com/terms.html` | 200 확인 |
> | 웹사이트 | massa-seven.vercel.app | `https://massaviet.com` | 200 확인 |
> | 계정 삭제 | massa-seven.vercel.app/delete-account.html | `https://app.massaviet.com/delete-account.html` | 200 확인 |
>
> 계정 삭제만 앱 주소를 쓴다. 소개 사이트에는 그 페이지가 없다 (404 확인).
>
> - [x] V6-0a. **Play Console — 2026-09-16 저장 완료** (제출은 안 함)
>       - 개인정보처리방침 URL → `https://massaviet.com/privacy.html`
>       - 데이터 보안 5단계 마법사 2단계의 '계정 URL 삭제' → `https://app.massaviet.com/delete-account.html`
>       - **아직 '게시 개요'에 저장만 된 상태다.** 프로덕션 검토가 끝난 뒤 vi·en 등록정보 2건과 함께 제출한다.
> - [ ] V6-0b. **App Store Connect — 새 버전이 있어야 바꿀 수 있다.**
>       개인정보 처리방침 URL 은 `앱이 수집하는 개인정보` 화면에 있는데,
>       "개인정보 처리방침을 변경하려면 새로운 앱 버전을 생성하십시오" 로 잠겨 있다.
>       1.0.7 이 막 출시돼 편집 가능한 버전이 없다. **1.0.8 에 반드시 함께 싣는다.**
>       (마케팅·지원 URL 은 1.0.7 에서 이미 교체했다. 남은 건 이 한 줄뿐이다.)
>       현재 값: `https://massa-seven.vercel.app/privacy.html`

> ### Vercel 종료 차단 조건
> **V6-0b 가 끝나기 전에는 Vercel 을 끌 수 없다.** 애플 제품 페이지의 개인정보 처리방침
> 링크가 죽는다. 다음 버전을 올릴 때 이 항목을 빼먹지 말 것.

- [ ] V6-1. Play 통계에서 구버전 설치 비중이 충분히 내려갔는지 확인
- [ ] V6-2. Vercel 프로젝트를 리디렉트만 남기거나 종료
- [x] V6-3a. `scripts/health_check.sh` 가 새 주소도 지켜보게 수정 — 2026-09-16
- [ ] V6-3b. 문서 4개의 massa-seven.vercel.app 문자열 정리
      (`스토어등록_정보.md` · `테스터_모집안내.md` · `PLAY_STORE_진행상황.md` · `store-assets/ds-import.csv`)
