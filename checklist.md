...
### 작업
- [ ] M1. FAQ 구조를 `BOT_FAQ` 배열로 바꾼다 — 규칙마다 `k:{ko,vi,en,zh,ja}` 키워드 + `a:` 한국어 답변
  - 매칭은 **5개 언어 키워드 전부**를 대상으로 한다(하노이 한국인이 영어로 칠 수도 있다). 비용 0
  - verify: 언어별 질문 매칭 시뮬레이션, 각 언어 대표 질문이 기대 규칙에 걸릴 것
- [ ] M2. 퀵리플라이 — 표시 `T(ko)`, 전달 `ko` 로 분리
  - verify: 5개 언어에서 칩을 눌렀을 때 `parseSearch` 결과가 한국어일 때와 동일할 것
- [ ] M3. 봇 문구 20개 + 신규 문구를 `scripts/i18n-add.py` 로 DICT 등록
  - verify: `trOne()` 로 20개 × 4개 언어 조회, 원문 반환(=미등록) 0건
- [ ] M4. 가격 답변을 `CACHE` 실데이터로 계산 (하드코딩 제거)
  - verify: DB 최저가와 봇 답변 숫자 일치
- [ ] M5. 취소 답변을 `CANCEL_RULES` 상수에서 생성 (3회/5회/7일/30일)
  - verify: 사이트 guide·faq·terms 와 문구 일치
- [ ] M6. 문의 경로 복구 — 회사 소개를 연락처 포함 화면으로, 봇 답변도 실제 주소 안내
- [ ] M7. Cloudflare Email Routing 으로 `support@massaviet.com` 수신 개통
  - verify: MX 3개 조회 + 실제 수신 테스트. **수신 주소 확인 메일 클릭은 사장님이 해야 한다**
- [ ] M8. 배포(`vps-deploy-web.sh`) 후 실기기·5개 언어 확인
